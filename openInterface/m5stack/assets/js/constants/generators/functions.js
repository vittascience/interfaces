...
def get_gps_frame():
  frame = str(uart.read())
  if frame.find("$") != -1:
    signal = frame.split("$")[1]
    if signal.find(",") != -1:
      if signal.split(",")[0] == "GNRMC":
        single_frame = signal.split(",")
        # frame - latitude - longitude - gps_time - speed - course
        return [signal,single_frame[3],single_frame[5],single_frame[1],single_frame[7],single_frame[8]]
      else:
        return ['']*6
    else:
      return ['']*6
  else:
    return ['GPS not found']*6`,
...
`def getRoomTemp(pinA1):
  ...
  res = (${READ_ANALOG_MAX_VALUE}.0-a) * 10000/a
...
`def getThmcTemp(pinA0, tempRoom):
  vout = getAnalogMean(pinA0)/${READ_ANALOG_MAX_VALUE}.0 * 5 * 1000
  vol  = (vout-350) / 54.16
