...
// https://github.com/ternjs/acorn => deprecated (use babelParser.js instead)


import  parse  from "./../external/babelParser/babelParser.js"
...
function simple(node, visitors, base1, state, override) {
  if (!base1) base1 = base
...
function ancestor(node, visitors, base1, state) {
  if (!base1) base1 = base
  let ancestors = []
  ;(function c(node, st, override) {
    let type = override || node.type, found = visitors[type]
...
// nodes).
function recursive(node, state, funcs, base1, override) {
  if (!base1) base1 = base
...
function findNodeAt(node, start, end, test, base1, state) {
  test = makeTest(test)
  if (!base1) base1 = base
  try {
    ;(function c(node, st, override) {
...
function findNodeAround(node, pos, test, base1, state) {
...
function findNodeAfter(node, pos, test, base1, state) {
  test = makeTest(test)
  if (!base1) base1 = base
  try {
    ;(function c(node, st, override) {
...
function findNodeBefore(node, pos, test, base1, state) {
  test = makeTest(test)
  if (!base1) base1 = base
  let max
  ;(function c(node, st, override) {
...
///////////////////////////
function walk1(ast, comments, block_loc, options){
  var st = {};
  var funcs = {};
  options = options || {}
  comments = comments || []
  ...
  var xml1 = document.createElement('xml');
  var root_node = xml1;
    ...
    if(name === 'block'){
      ...
      while(i<comments.length && comments[i].loc.start.line <= ast_node.loc.start.line){
        if(first){
          ...
          comm1 = document.createElement('comment')
  ...
  funcs.SwitchStatement = (node, st, c) => {
    ...
    for (let i = 0; i < node.cases.length-hasDefault; ++i) {
      let item = newNode('value',{name:'items'+(i)})
  ...
  funcs.BinaryExpression = (node, st, c) => {
    ...
    switch(node.operator){
      ...
      case '===': op='STRICT_EQ'; type = 'bi_logic_compare'; break;
      case '!=': op='NEQ'; type = 'bi_logic_compare'; break;
      case '!==': op='STRICT_NEQ'; type = 'bi_logic_compare'; break;     
  ...
  funcs.NewExpression = (node, st, c) => {
    ...
    var mutation1 = newNode('mutation', {items:node.arguments.length, names:''}) // TODO: Take out +1 when AddSub... is fixed on the actual length required.
    ...
    var node1 = current_node;
    if (node.arguments) for (let i = 0; i < node.arguments.length; ++i){
      let value2 = newNode('value', {name:'items'+(i)});
  ...
  funcs.ImportDeclaration = (node, st, c) => {
    ...
    current_node.appendChild(block1);
    var node1 = current_node;
    var mutation1 = newNode('mutation', {items:node.specifiers.length}) // TODO: Take out +1 when items count corrected on AddSub block
    block1.appendChild(mutation1)
    for (let i = 0; i < node.specifiers.length; i++){
      let item = newNode('value',{name:'items'+(i)})
      block1.appendChild(item)
...
function clearWorkspace(trashItems, workSpaceTarget) {
  if (trashItems === false) {
...
function updateWorkspace(e){
  ...
  //if(!window._BIDE.b2c_error){
    const blockly_code = Blockly.JavaScript.workspaceToCode(Blockly.getMainWorkspace());
...
function parseCode(code, workspaceElement){
  let Blockly = window.Blockly;
  let workspace = workspaceElement
  let ast1, xml1;
  let options;
  let comments = [];
  let block_loc = [];
  try{
    ...
    // replaced acorn by babel/parser
    ast1 = parse(code, options);
    // console.log(ast1);
    xml1 = walk1(ast1.program, ast1.comments, block_loc);
    // console.log(xml1);
    // clearWorkspace(false, workspace);
...
// window.parseCodeTest = parseCode;

/**
 * @class Js2Block
 * @description Class to convert JavaScript code to Blockly blocks
**/
window.Js2Block = class Js2Block {
  static instance = null;
  constructor (workspace, editor){
    this.workspace = workspace;
    this.editor = editor;
    this.codeJs = '';
    if(Js2Block.instance){
      return Js2Block.instance;
    }
    Js2Block.instance = this;
  }
   ...
   * @return {blockly.Xml}
   **/
  reorganizeBlocks(xmlText){
    try {
      Blockly.Events.disable();
      const workspace = this.workspace;
        var allBlocks = workspace.getAllBlocks();

        // find script_tag block
        var scriptTagBlock = allBlocks.filter(block => block.type === 'script_tag');
        if (scriptTagBlock.length === 0) {
            console.error("Block 'script_tag' not found"); // trigger an error if script_tag block is not found
            Blockly.Events.enable();
            return;
        }
        if (scriptTagBlock.length > 1) {
          for (let i=1; i<scriptTagBlock.length; i++){
            scriptTagBlock[i].dispose();
          }
        }
        scriptTagBlock = scriptTagBlock[0];

        
        var statementConnection = scriptTagBlock.getInput('content').connection;
        var childBlock = statementConnection.targetBlock();

        // supress all blocks in script_tag (only one script_tag is allowed for now)
        while (childBlock) {
        ...
        var lastBlock = statementConnection.targetBlock();
        while (lastBlock && lastBlock.nextConnection && lastBlock.nextConnection.isConnected()) {
            lastBlock = lastBlock.nextConnection.targetBlock();
        }
        

        Array.from(xmlText.children).forEach(childXml => {
          let childBlock = Blockly.Xml.domToBlock(childXml, workspace);
          ...
          if (lastBlock) {
              ...
              statementConnection.connect(childBlock.previousConnection);
          }
  ...
  * @param {string} code
  * @return {blockly.Xml}
  * @description : Convert JavaScript code to Blockly blocks
  **/
  addJsBlocks(code){
    const xml = parseCode(code, this.workspace);
    this.reorganizeBlocks(xml);
  };
