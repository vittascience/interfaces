...
Blockly.JavaScript.getelementbyid = function (block) {
  ...
  return ['document.getElementById(' + value_id + ')', Blockly.JavaScript.ORDER_NONE];
};

Blockly.JavaScript.getelementsbyclassname = function (block) {
  const value_class = Blockly.JavaScript.valueToCode(block, 'CLASS', Blockly.JavaScript.ORDER_ATOMIC);
  return ['document.getElementsByClassName(' + value_class + ')', Blockly.JavaScript.ORDER_NONE];
};

Blockly.JavaScript.getelementsbytagname = function (block) {
  const value_tag = Blockly.JavaScript.valueToCode(block, 'TAG', Blockly.JavaScript.ORDER_ATOMIC);
  return ['document.getElementsByTagName(' + value_tag + ')', Blockly.JavaScript.ORDER_NONE];
};

Blockly.JavaScript.addeventlistener = function (block) {
  const dropdown_event = block.getFieldValue('EVENT');
  const value_id = Blockly.JavaScript.valueToCode(block, 'ID', Blockly.JavaScript.ORDER_ATOMIC);
  const statements_name = Blockly.JavaScript.statementToCode(block, 'DO') || '';
  return value_id + '.addEventListener("' + dropdown_event + '", function(){' + NEWLINE + statements_name + '});' + NEWLINE;
};

Blockly.JavaScript.addeventlistenerOnLoad = function (block) {
  const statements_name = Blockly.JavaScript.statementToCode(block, 'DO') || '';
  return 'document.addEventListener("DOMContentLoaded", function(){' + NEWLINE + statements_name + '});' + NEWLINE;
};

Blockly.JavaScript.socket_emit_message = function (blocks) {
  const id = block.getFieldValue("ID");
  const message = Blockly.JavaScript.valueToCode(block, 'MESSAGE', Blockly.JavaScript.ORDER_ATOMIC) || "null";
  if (message == "null") {
    return "socket.emit('" + id + "', {});" + NEWLINE;
  }
  return "socket.emit('" + id + "', { data:" + message + "});" + NEWLINE;
};

Blockly.JavaScript.socket_on_message_receive = function (block) {
  const id = block.getFieldValue("ID");
  const dataVar = Blockly.JavaScript.nameDB_.getName(block.getFieldValue("VAR"), Blockly.VARIABLE_CATEGORY_NAME);
  const branchCode = Blockly.Python.statementToCode(block, "DO") || '';
  return "socket.on('" + id + "', (" + dataVar + ") => {" + NEWLINE + branchCode + "});" + NEWLINE;
};

Blockly.JavaScript.editattribut = function (block) {
  const value_id = Blockly.JavaScript.valueToCode(block, 'ID', Blockly.JavaScript.ORDER_ATOMIC);
  const text_attribut = block.getFieldValue('ATTRIBUT');
  const value_value = Blockly.JavaScript.valueToCode(block, 'VALUE', Blockly.JavaScript.ORDER_ATOMIC);
  return value_id + '.setAttribute("' + text_attribut + '", ' + value_value + ');' + NEWLINE;
};
