...
(function (root, factory) {
	...
	Blockly.JavaScript.lists_getSublist = function (a) {
		...
		if ('FIRST' == c && 'LAST' == d) b += '.slice(0)';
			...
			b =
				Blockly.JavaScript.provideFunction_('subsequence' + g[c] + g[d], ['function ' + Blockly.JavaScript.FUNCTION_NAME_PLACEHOLDER_ + '(sequence' + ('FROM_END' == c || 'FROM_START' == c ? ', at1' : '') + ('FROM_END' == d || 'FROM_START' == d ? ', at2' : '') + ') {', '  var start = ' + f('sequence', c, 'at1') + ';', '  var end = ' + f('sequence', d, 'at2') + ' + 1;', '  return sequence.slice(start, end);', '}']) +
				'(' +
				b +
	...
	Blockly.JavaScript.controls_for = function (a) {
		var b = Blockly.JavaScript.valueToCode(a,'VAR', Blockly.JavaScript.ORDER_ASSIGNMENT) || 'var1',
		...
		if (Blockly.isNumber(c) && Blockly.isNumber(d) && Blockly.isNumber(e)) {
			...
			(a = ''),
				(g = c),
				c.match(/^\w+$/) || Blockly.isNumber(c) || ((g = Blockly.JavaScript.nameDB_.getDistinctName(b + '_start', Blockly.VARIABLE_CATEGORY_NAME)), (a += 'var ' + g + ' = ' + c + ';\n')),
				(c = d),
				d.match(/^\w+$/) || Blockly.isNumber(d) || ((c = Blockly.JavaScript.nameDB_.getDistinctName(b + '_end', Blockly.VARIABLE_CATEGORY_NAME)), (a += 'var ' + c + ' = ' + d + ';\n')),
	...
	Blockly.JavaScript.math_change = function (a) {
		...
		const varName = Blockly.JavaScript.valueToCode(a,'VAR', Blockly.JavaScript.ORDER_ASSIGNMENT) || 'var1';
    console.log(varName);
		return varName + ' = (typeof ' + varName + " == 'number' ? " + varName + ' : 0) + ' + b + ';\n';
	};
	...
	Blockly.JavaScript.text_append = function (a) {
		var b = Blockly.JavaScript.valueToCode(a,'VAR', Blockly.JavaScript.ORDER_ASSIGNMENT) || '';
	...
	Blockly.JavaScript.text_getSubstring = function (a) {
		...
		if (e.match(/^'?\w+'?$/) || d) {
			...
			var f = Blockly.JavaScript.text.getIndex_,
				g = { FIRST: 'First', LAST: 'Last', FROM_START: 'FromStart', FROM_END: 'FromEnd' };
			b =
				Blockly.JavaScript.provideFunction_('subsequence' + g[b] + g[c], ['function ' + Blockly.JavaScript.FUNCTION_NAME_PLACEHOLDER_ + '(sequence' + ('FROM_END' == b || 'FROM_START' == b ? ', at1' : '') + ('FROM_END' == c || 'FROM_START' == c ? ', at2' : '') + ') {', '  var start = ' + f('sequence', b, 'at1') + ';', '  var end = ' + f('sequence', c, 'at2') + ' + 1;', '  return sequence.slice(start, end);', '}']) +
				'(' +
				e +
				('FROM_END' == b || 'FROM_START' == b ? ', ' + d : '') +
