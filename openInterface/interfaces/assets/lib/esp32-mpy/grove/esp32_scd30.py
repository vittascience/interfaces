...
class SCD30:
  ...
  def __init__(self, i2c, addr=_SCD30_DEFAULT_I2C_ADDR, pause=1000):
    if i2c == None:
  ...
  def start_continous_measurement(self, ambient_pressure=0):
    ...
    data = bint + bytes([crc])
    self._i2c.writeto_mem(self.addr, self.START_CONT_MEASURE, data, addrsize=16)
  ...
  def set_measurement_interval(self, interval):
    ...
    crc = self.__crc(bint[0], bint[1])
    data = bint + bytes([crc])
    self._i2c.writeto_mem(self.addr, self.SET_MEASURE_INTERVAL, data, addrsize=16)
  ...
  def set_automatic_recalibration(self, enable):
    ...
    crc = self.__crc(bint[0], bint[1])
    data = bint + bytes([crc])
    self._i2c.writeto_mem(self.addr, self.SET_FRC, data, addrsize=16)
  ...
  def set_temperature_offset(self, offset):
    ...
    crc = self.__crc(bint[0], bint[1])
    data = bint + bytes([crc])
    self._i2c.writeto_mem(self.addr, self.SET_TEMP_OFFSET, data, addrsize=16)
  ...
  def set_altitude_comp(self, altitude):
    ...
    crc = self.__crc(bint[0], bint[1])
    data = bint + bytes([crc])
    self._i2c.writeto_mem(self.addr, self.SET_ALT_COMP, data, addrsize=16)

  def __write_command(self, cmd):
    bcmd = struct.pack('>H', cmd)
    self._i2c.writeto(self.addr, bcmd)

  def __read_bytes(self, cmd, count):
    self.__write_command(cmd)
    utime.sleep_us(self.pause)
    return self._i2c.readfrom(self.addr, count)
