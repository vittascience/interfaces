...
class MLX90621:
    ...
    def mlx90621_read_ir(self, unit = 'int'):
        """
        ...
        for i in range(4):
            for j in range(16):
                ...
                self.temperatures_int[i][j] = int(to) # Returned as integers (tenths of a degree)
                self.temperatures_flt[i][j] = round(to, 1)
        if unit == 'int':
            return self.temperatures_int
        elif unit == 'float':
            return self.temperatures_flt

    def get_min_temp(self, unit='float'):
        """
        Retourne la température minimale du tableau IR
        unit: 'float' pour des degrés flottants, 'int' pour des entiers
        """
        arr = self.mlx90621_read_ir(unit)
        return min(val for row in arr for val in row)

    def get_max_temp(self, unit='float'):
        """
        Retourne la température maximale du tableau IR
        unit: 'float' pour des degrés flottants, 'int' pour des entiers
        """
        arr = self.mlx90621_read_ir(unit)
        return max(val for row in arr for val in row)

    def get_avg_temp(self, unit='float'):
        """
        Retourne la température moyenne du tableau IR
        unit: 'float' pour des degrés flottants, 'int' pour des entiers
        """
        arr = self.mlx90621_read_ir(unit)
        flat = [val for row in arr for val in row]
        avg = sum(flat) / len(flat)
        return round(avg, 1) if unit == 'float' else int(avg)