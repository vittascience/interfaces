...
class SI1145(object):
  ...
  def _read8(self, reg):
    return unpack('B', self._i2c.readfrom_mem(self._addr, reg, 1))[0] & 0xFF
 
  def _read16(self, reg, little_endian=True):
    result = unpack('BB', self._i2c.readfrom_mem(self._addr, reg, 2))
  ...
  def _write_param(self, param, val):
    self._write8(0x17, val)
    self._write8(0x18, param | 0xA0)
