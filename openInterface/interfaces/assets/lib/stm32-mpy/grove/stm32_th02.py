...
class TH02(object):
  ...
  def init_humidity(self):
    ...
    self._i2c.writeto_mem(self._addr, self.REGISTER_CONFIG, bytearray([self.CONVERSION_HUMIDITY]))

  def is_ready(self):
    """Is sensor done with conversion"""
    status = self.buf = self._i2c.readfrom_mem(self._addr, self.REGISTER_STATUS, 1)
    # Extract ready bit
    return bool(not (status & 0x01))

  def wait_until_ready(self):
    """Wait until conversion completes"""
    utime.sleep(self.CHECK_DELAY)
    for _ in range(8):
      if self.is_ready():
        return True
      utime.sleep(self.CHECK_DELAY)
  ...
  def read_data(self):
    ...
    data = self._i2c.readfrom_mem(self.I2C_ADDRESS, self.REGISTER_DATAH, 2)
  ...
  def get_temperature(self):
    ...
    self._i2c.writeto_mem(self._addr, self.REGISTER_CONFIG, bytearray([self.CONVERSION_TEMP]))
