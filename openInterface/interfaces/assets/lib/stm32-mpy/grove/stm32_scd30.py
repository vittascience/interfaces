# This file contains source code for which authorship could not be fully determined,
# despite reasonable efforts to identify the original creators.
# Rights are reserved for the rightful authors or their legal successors.
# If you believe you hold rights to any portion of this code, please contact us;
# we will take appropriate action, including removal if necessary.
...
class SCD30(object):
  ...
  def __init__(self, i2c, addr=SCD30_I2C_ADDR, pause=1000):
    if i2c == None:
      raise ValueError("I2C object 'SCD30' needed as argument!")
    self._i2c = i2c
  ...
  def start_continous_measurement(self, ambient_pressure=0):
    bint = pack('>H', ambient_pressure)
    crc = self.__crc(bint[0], bint[1])
    data = bint + bytes([crc])
    self._i2c.writeto_mem(self._addr, self.START_CONT_MEASURE, data, addrsize=16)
  ...
  def read_measurement(self):
    ...
    value = measurement[0:]
    co2 = unpack('>f', value[0:2] + value[3:5])[0]
    value = measurement[6:]
    temperature = unpack('>f', value[0:2] + value[3:5])[0]
    value = measurement[12:]
    relh = unpack('>f', value[0:2] + value[3:5])[0]
  ...
  def get_temperature_offset(self):
    ...
    self.__check_crc(bint)
    return unpack('>H', bint)[0] / 100.0

  def set_temperature_offset(self, offset):
    bint = pack('>H', int(offset * 100))
    crc = self.__crc(bint[0], bint[1])
    data = bint + bytes([crc])
    self._i2c.writeto_mem(self._addr, self.SET_TEMP_OFFSET, data, addrsize=16)
  ...
  def set_altitude_comp(self, altitude):
    bint = pack('>H', altitude)
    crc = self.__crc(bint[0], bint[1])
    data = bint + bytes([crc])
    self._i2c.writeto_mem(self._addr, self.SET_ALT_COMP, data, addrsize=16)

  def __write_command(self, cmd):
    bcmd = pack('>H', cmd)
    self._i2c.writeto(self._addr, bcmd)
