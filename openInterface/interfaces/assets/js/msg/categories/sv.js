/**
 * @fileoverview English translation file for interfaces Toolbox. (EN)
 */

Blockly.MESSAGES = {
    title: "Code",
    blocks: "Blocks",
    linkTooltip: "Save and link to blocks.",
    runTooltip: "Run the program defined by the blocks in the workspace.",
    badCode: "Program error:\n%1",
    timeout: "Maximum execution iterations exceeded.",
    trashTooltip: "Discard all blocks.",
    httpRequestError: "There was a problem with the request.",
    linkAlert: "Share your blocks with this link:\n\n%1",
    hashError: "Sorry, '%1' doesn't correspond with any saved program.",
    xmlError: "Could not load your saved file. Perhaps it was created with a different version of Blockly?",
    badXml: "Error parsing XML:\n%1\n\nSelect 'OK' to abandon your changes or 'Cancel' to further edit the XML.",
    search: "Search a block",
    setLevel: "Sort by level",
    textVariable: "text",
    listVariable: "list",
    hello: "Hello !",
    comment: "This code part serves to ...",
    data1: "Data1",
    data2: "Data2",
    setText: "Set a text :",
    setNumber: "Set a number :",
    radioMessage: "I'm the radio message !",
    // For Esp32/Pico/Galaxia/M5Stack
    webPage_title: "Esp32 Server",
    buttonName: "button_id",
    sliderName: "slider_id",
    switchName: "switch_id",
    topicName: "topic_name",
    base64Image: '',
    // For M5Stack
    m5title: 'Title',
    m5text: 'Text',
    // For CyberPi
    hello_cyberpi: 'Welcome on CyberPi !',
    label: 'label',
    // For Python
    modelPath : "https://vittascience.com/ia/model/644237cc1072c/",
    modelSoundPath : "https://vittascience.com/ia/model/64413850632bb/",
    modelPosturePath: 'https://vittascience.com/ia/model/66043c5502768/'
};

// Toolbox basic category names.
// Toolbox hardware category names.
// Toolbox scratch mode
// Specific categories

// Toolbox hardware subcategories
// DISPLAY
// IO
// COMMUNICATION
// NETWORK
// SENSORS
// ACTUATORS
// ROBOTS
// IA
// Others

// Arduino

// BBC micro:bit

// Python

// WB55 - L476
// L476
// WB55

// Nao
// Nyrio

// Pico

// Buddy

// ESP32

// Thymio

// Cyberpi

// Eliobot

// Galaxia

// M5Stack

// Raspberry Pi

// Galaxia CP

// Codey & Rocky

// mBot
Blockly.Msg['CATEGORY_LOGIC'] = 'Logik';
Blockly.Msg['CATEGORY_LOOPS'] = 'Loopar';
Blockly.Msg['CATEGORY_MATH'] = 'Matematik';
Blockly.Msg['CATEGORY_TEXT'] = 'Text';
Blockly.Msg['CATEGORY_VARIABLES'] = 'Variabler';
Blockly.Msg['CATEGORY_LISTS'] = 'Listor';
Blockly.Msg['CATEGORY_PROCEDURES'] = 'Funktioner';
Blockly.Msg['CATEGORY_EXCEPTION'] = 'Undantag';
Blockly.Msg['CATEGORY_COLOUR'] = 'Färg';
Blockly.Msg['CATEGORY_DICTIONARIES'] = 'Ordböcker';
Blockly.Msg['CATEGORY_DISPLAY'] = 'Skärm';
Blockly.Msg['CATEGORY_IO'] = 'In- och utmatning';
Blockly.Msg['CATEGORY_COMMUNICATION'] = 'Kommunikation';
Blockly.Msg['CATEGORY_NETWORK'] = 'Nätverk';
Blockly.Msg['CATEGORY_SENSORS'] = 'Sensorer';
Blockly.Msg['CATEGORY_ACTUATORS'] = 'Ställdon';
Blockly.Msg['CATEGORY_ROBOTS'] = 'Robotar';
Blockly.Msg['CATEGORY_ROBOT'] = 'Robot';
Blockly.Msg['CATEGORY_VITTAIA'] = 'AI';
Blockly.Msg['CATEGORY_TIME'] = 'Tid';
Blockly.Msg['CATEGORY_APPEARANCE'] = 'Utseende';
Blockly.Msg['CATEGORY_SOUND'] = 'Ljud';
Blockly.Msg['CATEGORY_CONTROL'] = 'Styrning';
Blockly.Msg['CATEGORY_OPERATORS'] = 'Operatorer';
Blockly.Msg['CATEGORY_EVENTS'] = 'Händelser';
Blockly.Msg['CATEGORY_MOVEMENT'] = 'Rörelse';
Blockly.Msg['CATEGORY_UTILITY'] = 'Hjälpverktyg';
Blockly.Msg['CATEGORY_GAME'] = 'Spel';
Blockly.Msg['CATEGORY_TOOL'] = 'Verktyg';
Blockly.Msg['CATEGORY_PROCESS'] = 'Process';
Blockly.Msg['CATEGORY_VISION'] = 'Datorseende';
Blockly.Msg['CATEGORY_VOICE_INTERACTIONS'] = 'Röst';
Blockly.Msg['CATEGORY_CAMERA'] = 'Kamera';
Blockly.Msg['CATEGORY_TURTLE'] = 'Turtle';
Blockly.Msg['CATEGORY_GRAPHICS'] = 'Grafik';
Blockly.Msg['CATEGORY_NUMPY'] = 'Numpy';
Blockly.Msg['CATEGORY_ACTIONS'] = 'Åtgärder';
Blockly.Msg['CATEGORY_SCREEN'] = 'Skärm';
Blockly.Msg['CATEGORY_DRONE'] = 'Drönare';
Blockly.Msg['CATEGORY_MUSIC'] = 'Musik';
Blockly.Msg['CATEGORY_MCORE'] = 'mCore';
Blockly.Msg['CATEGORY_BACKPACK'] = 'Ryggsäck';
Blockly.Msg['SUBCATEGORY_DISPLAY'] = 'Skärm';
Blockly.Msg['SUBCATEGORY_DISPLAYS_LCD'] = 'LCD-skärm';
Blockly.Msg['SUBCATEGORY_DISPLAYS_OLED'] = 'OLED-skärm';
Blockly.Msg['SUBCATEGORY_LED'] = 'LED';
Blockly.Msg['SUBCATEGORY_NEOPIXEL'] = 'Neopixel';
Blockly.Msg['SUBCATEGORY_DISPLAYS_LED_MATRIX'] = 'LED-matris';
Blockly.Msg['SUBCATEGORY_DISPLAYS_RGB_LED_MATRIX'] = 'RGB LED-matris';
Blockly.Msg['SUBCATEGORY_CHAINABLE_LED_RGB'] = 'Kedjbar RGB-LED';
Blockly.Msg['SUBCATEGORY_IO'] = 'In- och utmatning';
Blockly.Msg['SUBCATEGORY_TIME'] = 'Tid';
Blockly.Msg['SUBCATEGORY_EXTERNAL_INPUTS'] = 'Externa ingångar';
Blockly.Msg['SUBCATEGORY_PINS'] = 'Stift';
Blockly.Msg['SUBCATEGORY_EVENTS'] = 'Händelser';
Blockly.Msg['SUBCATEGORY_COMMUNICATION'] = 'Kommunikation';
Blockly.Msg['SUBCATEGORY_SERIAL_CONNECTION'] = 'Seriell anslutning';
Blockly.Msg['SUBCATEGORY_GPS'] = 'GPS';
Blockly.Msg['SUBCATEGORY_NFC'] = 'NFC';
Blockly.Msg['SUBCATEGORY_LORA'] = 'LoRa';
Blockly.Msg['SUBCATEGORY_DATA_LOGGING'] = 'Datainsamling';
Blockly.Msg['SUBCATEGORY_INTEGRATED_BT'] = 'Integrerad Bluetooth';
Blockly.Msg['SUBCATEGORY_EXTERNAL_BLUETOOTH'] = 'Externa Bluetooth-moduler';
Blockly.Msg['SUBCATEGORY_TRACKING_MODULES'] = 'Spårningsmoduler';
Blockly.Msg['SUBCATEGORY_UART'] = 'UART';
Blockly.Msg['SUBCATEGORY_WIRELESS_COMMUNICATION'] = 'Trådlös kommunikation';
Blockly.Msg['SUBCATEGORY_IR_COMMUNICATION'] = 'Infraröd kommunikation';
Blockly.Msg['SUBCATEGORY_INTEGRATED_RADIO'] = 'Integrerad radio';
Blockly.Msg['SUBCATEGORY_WIFI'] = 'WiFi';
Blockly.Msg['SUBCATEGORY_SERVER'] = 'Server';
Blockly.Msg['SUBCATEGORY_CLIENT'] = 'Klient';
Blockly.Msg['SUBCATEGORY_WEB_PAGE'] = 'Skapa webbsida';
Blockly.Msg['SUBCATEGORY_DATA_WEB_PAGE'] = 'Webbsidans data';
Blockly.Msg['SUBCATEGORY_HTTP'] = 'HTTP';
Blockly.Msg['SUBCATEGORY_UMAIL'] = 'E-post';
Blockly.Msg['SUBCATEGORY_MQTT'] = 'MQTT';
Blockly.Msg['SUBCATEGORY_SENSORS'] = 'Sensorer';
Blockly.Msg['SUBCATEGORY_SENSORS_GAS'] = 'Gassensorer';
Blockly.Msg['SUBCATEGORY_SENSORS_CLIMATE'] = 'Klimat- och vädersensorer';
Blockly.Msg['SUBCATEGORY_SENSORS_SOUNDLIGHT'] = 'Ljud- och ljussensorer';
Blockly.Msg['SUBCATEGORY_SENSORS_DISTANCEMOVEMENT'] = 'Avstånds- och rörelsesensorer';
Blockly.Msg['SUBCATEGORY_SENSORS_HEALTH'] = 'Hälsosensorer';
Blockly.Msg['SUBCATEGORY_SENSORS_OTHER'] = 'Andra sensorer';
Blockly.Msg['SUBCATEGORY_ACTUATORS'] = 'Ställdon';
Blockly.Msg['SUBCATEGORY_MOTORS'] = 'Motorer';
Blockly.Msg['SUBCATEGORY_FREE_MOVEMENTS'] = 'Fria rörelser';
Blockly.Msg['SUBCATEGORY_MOVE_BY_SQUARES'] = 'Flytta per ruta';
Blockly.Msg['SUBCATEGORY_MUSIC'] = 'Musik';
Blockly.Msg['SUBCATEGORY_MOSFET'] = 'MOSFET';
Blockly.Msg['SUBCATEGORY_ACTUATORS_OTHER'] = 'Andra ställdon';
Blockly.Msg['SUBCATEGORY_SERVOMOTORS'] = 'Servomotorer';
Blockly.Msg['SUBCATEGORY_DETECTION'] = 'Detektion';
Blockly.Msg['SUBCATEGORY_CAMERA'] = 'Kamera';
Blockly.Msg['SUBCATEGORY_CAMERAS'] = 'Kameror';
Blockly.Msg['SUBCATEGORY_SENSOR_DATA'] = 'Sensordata';
Blockly.Msg['SUBCATEGORY_VITTAIA_SENSOR_DATA'] = 'AI-sensordata';
Blockly.Msg['SUBCATEGORY_VITTAIA_IMAGE'] = 'AI-bild';
Blockly.Msg['SUBCATEGORY_VITTAIA_POSTURE'] = 'AI-hållning';
Blockly.Msg['SUBCATEGORY_VITTAIA_SOUND'] = 'AI-ljud';
Blockly.Msg['SUBCATEGORY_VITTAIA_TEXT'] = 'AI-text';
Blockly.Msg['SUBCATEGORY_LOGIC'] = Blockly.Msg['CATEGORY_LOGIC'];
Blockly.Msg['SUBCATEGORY_LOOPS'] = Blockly.Msg['CATEGORY_LOOPS'];
Blockly.Msg['SUBCATEGORY_ENCRYPTION'] = 'Kryptering';
Blockly.Msg['SUBCATEGORY_CONSOLE'] = 'Konsol';
Blockly.Msg['SUBCATEGORY_BUTTON'] = 'Knapp';
Blockly.Msg['SUBCATEGORY_INFRARED'] = 'Infraröd';
Blockly.Msg['SUBCATEGORY_MICROPHONE'] = 'Mikrofon';
Blockly.Msg['SUBCATEGORY_COMPUTER'] = 'Dator';
Blockly.Msg['SUBCATEGORY_BUILTIN_LED_MATRIX'] = 'Inbyggd LED-matris';
Blockly.Msg['SUBCATEGORY_I2C_MOTOR_DRIVER'] = 'I2C-motordrivare';
Blockly.Msg['SUBCATEGORY_MINI_I2C_MOTOR_DRIVER'] = 'Mini I2C-motordrivare';
Blockly.Msg['SUBCATEGORY_ARDUINO_SHILED_MOTOR'] = 'Arduino Motor Shield';
Blockly.Msg['SUBCATEGORY_MICROBIT'] = 'micro:bit';
Blockly.Msg['SUBCATEGORY_KITRONIK'] = "Kitronik";
Blockly.Msg['SUBCATEGORY_GAMES'] = 'Spel';
Blockly.Msg['SUBCATEGORY_EXEC'] = 'Kör Python-kod';
Blockly.Msg['SUBCATEGORY_MICROBIT_LOG'] = 'micro:bit-logg';
Blockly.Msg['SUBCATEGORY_ENVIRO_BIT'] = 'Enviro:bit';
Blockly.Msg['SUBCATEGORY_WEATHER_BIT'] = 'Weather:bit';
Blockly.Msg['SUBCATEGORY_KITRONIK_ENVIRONMENTAL'] = 'Kitronik Environmental Shield';
Blockly.Msg['SUBCATEGORY_SPEECH'] = 'Tal';
Blockly.Msg['SUBCATEGORY_AUDIO'] = 'Ljud';
Blockly.Msg['SUBCATEGORY_MAQUEEN'] = 'Maqueen Lite';
Blockly.Msg['SUBCATEGORY_MAQUEEN_PLUS'] = 'Maqueen Plus';
Blockly.Msg['SUBCATEGORY_LIDAR'] = 'LiDAR';
Blockly.Msg['SUBCATEGORY_LINE_FINDER'] = 'Linjeföljare';
Blockly.Msg['SUBCATEGORY_CONTROL'] = 'Styrning';
Blockly.Msg['SUBCATEGORY_CUTEBOT'] = 'Cutebot';
Blockly.Msg['SUBCATEGORY_CUTEBOT_PRO'] = 'Cutebot Pro';
Blockly.Msg['SUBCATEGORY_MOVING'] = 'Rörelse';
Blockly.Msg['SUBCATEGORY_RGB_LED'] = 'RGB-LED';
Blockly.Msg['SUBCATEGORY_PID'] = 'PID';
Blockly.Msg['SUBCATEGORY_REMOTE_CONTROL'] = 'Infraröd fjärrkontroll';
Blockly.Msg['SUBCATEGORY_MOTOR_PORT'] = 'Motorport (M)';
Blockly.Msg['SUBCATEGORY_KITROBOT'] = 'Kitro:bot v2';
Blockly.Msg['SUBCATEGORY_CODO'] = 'Codo';
Blockly.Msg['SUBCATEGORY_OOBYBOT'] = 'Oobybot';
Blockly.Msg['SUBCATEGORY_BUGGY'] = 'Buggy';
Blockly.Msg['SUBCATEGORY_BITBOT'] = 'Bit:Bot';
Blockly.Msg['SUBCATEGORY_BITCAR'] = 'BitCar';
Blockly.Msg['SUBCATEGORY_GAMEPAD'] = 'Spelkontroll';
Blockly.Msg['SUBCATEGORY_TELLO'] = 'Tello - Grundkommando';
Blockly.Msg['SUBCATEGORY_TELLO_SEND_COMMAND'] = 'Tello - Skicka kommando';
Blockly.Msg['SUBCATEGORY_TELLO_READ'] = 'Tello - Ta emot data';
Blockly.Msg['SUBCATEGORY_HUSKYLENS'] = 'HuskyLens';
Blockly.Msg['SUBCATEGORY_ZIP_HALO'] = 'Kitronik ZIP Halo HD';
Blockly.Msg['SUBCATEGORY_SUPERBIT_BASIC'] = 'Superbit - Grundläggande';
Blockly.Msg['SUBCATEGORY_SUPERBIT_SENSORS'] = 'Yahboom - Sensorer';
Blockly.Msg['SUBCATEGORY_MATPLOTLIB'] = 'Matplotlib';
Blockly.Msg['SUBCATEGORY_ARRAYS_MATRIX'] = 'Arrayer och matriser';
Blockly.Msg['SUBCATEGORY_OPERATIONS'] = 'Operationer';
Blockly.Msg['SUBCATEGORY_ALPHABOT_SENSORS'] = 'Alphabot - Sensorer';
Blockly.Msg['SUBCATEGORY_ALPHABOT_MOTORS'] = 'Alphabot - Motorer';
Blockly.Msg['SUBCATEGORY_ALPHABOT_OLED'] = 'Alphabot - OLED-skärm';
Blockly.Msg['SUBCATEGORY_ALPHABOT_COMMANDS'] = 'Alphabot - Styrning';
Blockly.Msg['SUBCATEGORY_ALPHABOT_LED_RGB'] = 'Alphabot - RGB-LED';
Blockly.Msg['SUBCATEGORY_RTC'] = 'Realtidsklocka (inbyggd modul)';
Blockly.Msg['SUBCATEGORY_EXPANSION_IKS01A3'] = 'IKS01A3 expansionskort';
Blockly.Msg['SUBCATEGORY_L476'] = 'L476';
Blockly.Msg['SUBCATEGORY_DONUTBOT_SENSORS'] = 'Donutbot - Sensorer';
Blockly.Msg['SUBCATEGORY_DONUTBOT_MOTORS'] = 'Donutbot - Motorer';
Blockly.Msg['SUBCATEGORY_DONUTBOT_LED_RGB'] = 'Donutbot - RGB-LED';
Blockly.Msg['SUBCATEGORY_DONUTBOT_IO'] = 'Donutbot - In-/utgångar';
Blockly.Msg['SUBCATEGORY_DONUTBOT_COMMUNICATION'] = 'Donutbot - Kommunikation';
Blockly.Msg['SUBCATEGORY_WB55'] = 'WB55';
Blockly.Msg['SUBCATEGORY_STEAMI'] = 'STeaMi';
Blockly.Msg['SUBCATEGORY_BLUETOOTH_BLE'] = 'Bluetooth (inbyggd BLE-modul)';
Blockly.Msg['SUBCATEGORY_ANIMATED_SPEECH'] = 'Animerat tal';
Blockly.Msg['SUBCATEGORY_ASR'] = 'Taligenkänning';
Blockly.Msg['SUBCATEGORY_CAPITAL_CITY'] = 'Huvudstäder och länder';
Blockly.Msg['SUBCATEGORY_STORY_TELLING'] = 'Interaktiv berättelse';
Blockly.Msg['SUBCATEGORY_MENTAL_CALCULATION'] = 'Huvudräkning';
Blockly.Msg['SUBCATEGORY_MOVEMENT_JOINTS'] = 'Leder';
Blockly.Msg['SUBCATEGORY_MOVEMENT_POSE'] = 'Positionering';
Blockly.Msg['SUBCATEGORY_TOOL'] = 'Verktyg';
Blockly.Msg['SUBCATEGORY_DISPLAY_LED_RING'] = 'LED-ring';
Blockly.Msg['SUBCATEGORY_ACTUATORS_CONVEYOR'] = 'Transportband';
Blockly.Msg['SUBCATEGORY_SENSOR_IR_CONVEYOR'] = 'IR-sensor (transportband)';
Blockly.Msg['SUBCATEGORY_UTILITIES'] = 'Hjälpverktyg';
Blockly.Msg['SUBCATEGORY_PICO_LED'] = 'Raspberry Pi Pico';
Blockly.Msg["SUBCATEGORY_KITRO"] = "Kitronik";
Blockly.Msg['SUBCATEGORY_OBJECT_DETECTION'] = 'Objektigenkänning';
Blockly.Msg['SUBCATEGORY_MOTION_DETECTION'] = 'Rörelseavkänning';
Blockly.Msg['SUBCATEGORY_PERSON_TRACKING'] = 'Personspårning';
Blockly.Msg['SUBCATEGORY_VISION'] = 'Datorsyn';
Blockly.Msg['SUBCATEGORY_FACE'] = 'Ansikte';
Blockly.Msg['SUBCATEGORY_DISTANCE'] = 'Avstånd';
Blockly.Msg['SUBCATEGORY_TOUCH'] = 'Beröringssensor';
Blockly.Msg['SUBCATEGORY_BATTERY'] = 'Batteri';
Blockly.Msg['SUBCATEGORY_MOTION_SENSOR'] = 'Rörelsesensor';
Blockly.Msg['SUBCATEGORY_COLOR'] = 'Färg';
Blockly.Msg['SUBCATEGORY_HEAD_MOVEMENTS'] = 'Huvudrörelser';
Blockly.Msg['SUBCATEGORY_WHEELS'] = 'Hjul';
Blockly.Msg['SUBCATEGORY_TALK'] = 'Tala';
Blockly.Msg['SUBCATEGORY_VOLUME'] = 'Ljudvolym';
Blockly.Msg['SUBCATEGORY_SPEECH_RATE'] = 'Talhastighet';
Blockly.Msg['SUBCATEGORY_PITCH'] = 'Röstton';
Blockly.Msg['SUBCATEGORY_ESP32'] = 'ESP32';
Blockly.Msg['SUBCATEGORY_ESP32_CAM'] = 'ESP32-kamera';
Blockly.Msg["SUBCATEGORY_ILO"] = "Ilo";
Blockly.Msg['SUBCATEGORY_ALVIK'] = 'Alvik';
Blockly.Msg['SUBCATEGORY_ALVIK_MOTORS'] = 'Alvik - Motorer';
Blockly.Msg['SUBCATEGORY_ALVIK_LEDS'] = 'Alvik - Lysdioder';
Blockly.Msg['SUBCATEGORY_ALVIK_SENSORS'] = 'Alvik - Sensorer';
Blockly.Msg['SUBCATEGORY_THYMIO'] = 'Thymio';
Blockly.Msg['SUBCATEGORY_SOUND'] = 'Ljud';
Blockly.Msg['SUBCATEGORY_SOUNDS'] = 'Ljudklipp';
Blockly.Msg['SUBCATEGORY_CYBERPI'] = 'CyberPi';
Blockly.Msg['SUBCATEGORY_TEXT'] = 'Text';
Blockly.Msg['SUBCATEGORY_CHART'] = 'Diagram';
Blockly.Msg['SUBCATEGORY_BUTTONS'] = 'Knappar';
Blockly.Msg['SUBCATEGORY_AUDIO'] = 'Ljud';
Blockly.Msg['SUBCATEGORY_ENCODER_MOTORS'] = 'Encoder-motorer';
Blockly.Msg['SUBCATEGORY_MOTORS_M1_M2'] = 'Motorer M1 & M2';
Blockly.Msg['SUBCATEGORY_ULTRASONIC_SENSORS'] = 'Ultraljudssensorer';
Blockly.Msg['SUBCATEGORY_QUAD_RGB_SENSORS'] = 'Fyra RGB-sensorer';
Blockly.Msg['SUBCATEGORY_ELIOBOT'] = 'Eliobot';
Blockly.Msg['SUBCATEGORY_BUILTIN_LED'] = 'Inbyggd LED';
Blockly.Msg['SUBCATEGORY_EYES'] = 'Ögon';
Blockly.Msg['SUBCATEGORY_LINE_SENSORS'] = 'Linjesensorer';
Blockly.Msg['SUBCATEGORY_BUZZER'] = 'Summer';
Blockly.Msg['SUBCATEGORY_IR_REMOTE'] = 'IR-fjärrkontroll';
Blockly.Msg['SUBCATEGORY_POTENTIOMETER'] = 'Potentiometer';
Blockly.Msg['SUBCATEGORY_GALAXIA'] = 'Galaxia';
Blockly.Msg['SUBCATEGORY_GALAXIA_DISPLAY'] = 'Galaxia - Skärm';
Blockly.Msg['SUBCATEGORY_GALAXIA_RGB_LED'] = 'Galaxia - RGB-LED';
Blockly.Msg['SUBCATEGORY_GALAXIA_GRAPHICS'] = 'Galaxia - Grafik';
Blockly.Msg['SUBCATEGORY_GALAXIA_A_B'] = 'Galaxia - Knappar A och B';
Blockly.Msg['SUBCATEGORY_GALAXIA_TOUCH_BUTTONS'] = 'Galaxia - Tryckknappar';
Blockly.Msg['SUBCATEGORY_INFRARED_COMMUNICATION'] = 'Infraröd kommunikation';
Blockly.Msg['SUBCATEGORY_GALAXIA_LOG'] = 'Galaxia - Logg';
Blockly.Msg['SUBCATEGORY_M5STACK'] = 'M5Stack';
Blockly.Msg['SUBCATEGORY_SETTINGS'] = 'Inställningar';
Blockly.Msg['SUBCATEGORY_DRAW_TITLE'] = 'Rita - Rubrik';
Blockly.Msg['SUBCATEGORY_DRAW_LABEL'] = 'Rita - Text';
Blockly.Msg['SUBCATEGORY_DRAW_RECT'] = 'Rita - Rektangel';
Blockly.Msg['SUBCATEGORY_DRAW_CIRCLE'] = 'Rita - Cirkel';
Blockly.Msg['SUBCATEGORY_DRAW_TRIANGLE'] = 'Rita - Triangel';
Blockly.Msg['SUBCATEGORY_DRAW_LINE'] = 'Rita - Linje';
Blockly.Msg['SUBCATEGORY_GRAPHICS'] = 'Grafik';
Blockly.Msg['SUBCATEGORY_SENSEHAT_JOYSTICK'] = 'Sense HAT - Joystick';
Blockly.Msg['SUBCATEGORY_SENSEHAT_SENSORS'] = 'Sense HAT - Sensorer';
Blockly.Msg['SUBCATEGORY_SENSEHAT_MATRIX'] = 'Sense HAT - LED-matris';
Blockly.Msg['MESSAGE_ROBOTS_USB_CAMERA'] = 'För att programmera USB-kameran finns blocken i kategorin \'Sensorer\'';
Blockly.Msg['CATEGORY_BASIC'] = 'Grundläggande';
Blockly.Msg['SUBCATEGORY_GALAXIA_TIME'] = 'Galaxia - Tid';
Blockly.Msg['SUBCATEGORY_GALAXIA_PINS'] = 'Galaxia - Stift P0, P1 och P2';
Blockly.Msg['SUBCATEGORY_MORPION'] = 'Tre i rad';
Blockly.Msg['SUBCATEGORY_WEB_SERVER'] = 'Webbserver';
Blockly.Msg['SUBCATEGORY_OTHER'] = 'Övriga';
Blockly.Msg['SUBCATEGORY_MOTION'] = 'Rörelse';
Blockly.Msg["SUBCATEGORY_MCORE_LED"] = "mCore - blå LED";
Blockly.Msg["SUBCATEGORY_MCORE_RGB_LED"] = "mCore - RGB-LED";
Blockly.Msg["SUBCATEGORY_MCORE_SENSORS"] = "mCore - Sensorer";
Blockly.Msg["SUBCATEGORY_MCORE_BUZZER"] = "mCore - Summer";
Blockly.Msg["SUBCATEGORY_MCORE_INFRARED"] = "mCore - Infraröd";
Blockly.Msg['SUBCATEGORY_LED_MATRIX'] = 'LED-matris';
Blockly.Msg['SUBCATEGORY_4DIGITS_DISPLAY'] = '7-segmentsdisplay';
Blockly.Msg['SUBCATEGORY_MBOT_MOTORS'] = 'Motorer';
Blockly.Msg['SUBCATEGORY_SERVOMOTOR'] = 'Servomotor';
Blockly.Msg['SUBCATEGORY_MINI_FAN'] = 'Mini-fläkt';
Blockly.Msg['SUBCATEGORY_TB6612FNG_MOTOR_DRIVER'] = "TB6612FNG-motordrivare";
