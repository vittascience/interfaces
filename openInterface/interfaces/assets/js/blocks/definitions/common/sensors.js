/**
 * @fileoverview Common Sensors blocks.
 */
...
Blockly.defineBlocksWithJsonArray([ // BEGIN JSON EXTRACT
    ...
    // BLOCK GROVE SUNLIGHT SENSOR _ READ LIGHT (I2C) SI1145/51
    {
...
Blockly.Constants.Sensors.SENSORS_ULTRASONIC_INIT_EXTENSION = function () {
    const dropdown = this.getField("SENSOR");
    dropdown.setValidator(function (value) {
        const newSensor = (value == "GROVE");
        if (newSensor != this.isGrove_) {
            const block = this.getSourceBlock();
            block.updateField_(newSensor);
        }
    });
    const isGroveInital = (this.getFieldValue("SENSOR") == "GROVE");
...
Blockly.Constants.Sensors.SENSORS_ULTRASONIC_MUTATOR_MIXIN = {
    ...
    domToMutation: function (xmlElement) {
        const isGrove = (xmlElement.getAttribute('pin') != 'false');
    ...
    updateField_: function (isGrove) {
        ...
        // Détermine les pins à utiliser
        const useShieldPins = typeof VittaInterface !== 'undefined' &&
            VittaInterface &&
            VittaInterface.shieldView &&
            typeof INTERFACE_BOARDS !== 'undefined' &&
            INTERFACE_NAME !== 'arduino';

        const pins = useShieldPins
            ? Blockly.Constants.Pins.digital[INTERFACE_BOARDS[Blockly.Constants.getSelectedBoard()].shieldId]
        ...
        if (isGrove) {
            ...
            this.appendDummyInput("HC_PINS")
                ...
                .appendField("TRIG")
                .appendField(new Blockly.FieldDropdown(pins), "TRIG")
                .appendField("ECHO")
