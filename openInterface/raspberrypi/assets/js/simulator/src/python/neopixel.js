// python/raspberry - neopixel module
...
var $builtinmodule = function (name) {
	...
	var NeoPixel = function ($gbl, $loc) {
        ...
        $loc.set_color = new Sk.builtin.func(function (self, r, g, b) {
            Sk.builtin.pyCheckArgsLen("set_color", arguments.length, 4, 4);
        ...
        $loc.setPixelColor = new Sk.builtin.func(function (self, index) {
            Sk.builtin.pyCheckArgsLen("setPixelColor", arguments.length, 2, 2);
            Sk.builtin.pyCheckType("index", "integer", Sk.builtin.checkInt(index));
            const indexValue = index.v;
            self.LEDS[index.v] = [self.r, self.g, self.b];
            if (typeof self.LEDS[indexValue] === 'object') {
                const color = self.LEDS[indexValue];
                $(".neopixel-" + self.pin).eq(indexValue).css("background-color", "rgb(" + color[0] + "," + color[1] + "," + color[2] + ")");
            }
        });
