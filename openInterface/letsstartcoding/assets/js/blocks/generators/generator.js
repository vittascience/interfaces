...
Blockly.Arduino.Generators.setupSerialConnection = function () {
    Blockly.Arduino.addFunction('serial_setupConnection', FUNCTIONS_LETSSTARTCODING.DEF_SETUP_SERIAL_CONNECTION)
    const baud = document.getElementById("baud");
    Blockly.Arduino.addSetup('setup_serial', "serial_setupConnection(" + baud.value + ");");
};
...
Blockly.Arduino.Generators.digital_read = function (pin, codeFlag) {
    ...
    if (codeFlag) {
        ...
        Blockly.Arduino.addSetup(pinName, "pinMode(" + constantName + ", INPUT);");
        return constantName;
    } else {
...
Blockly.Arduino.Generators.digital_write = function (pin, codeFlag) {
    ...
    if (codeFlag) {
        const constantName = "PIN_" + codeFlag.toUpperCase().replace(/ /g, '_') + '_' + pin;
        Blockly.Arduino.addDefine(pinName, "#define " + constantName + TAB + pin);
        Blockly.Arduino.addSetup(pinName, "pinMode(" + constantName + ", OUTPUT);");
        return constantName;
    } else {
...
Blockly.Arduino.Generators.analog_read = function (pin, codeFlag, addingAnalogMean = true) {
    ...
    const pinName = pins.find(p => p[1] == pin)[0];
    if (addingAnalogMean) {
        Blockly.Arduino.addFunction('getAnalogMean', FUNCTIONS_LETSSTARTCODING.DEF_GET_ANALOG_MEAN);
    }
    if (codeFlag) {
        const constantName = "PIN_" + codeFlag.toUpperCase().replace(/ /g, '_') + '_' + pin;
        Blockly.Arduino.addDefine(pinName, "#define " + constantName + TAB + pinName);
        Blockly.Arduino.addSetup(pinName, "pinMode(" + constantName + ", INPUT);");
        return constantName;
    } else {
        Blockly.Arduino.addSetup(pinName, "pinMode(" + pin + ", OUTPUT);");
