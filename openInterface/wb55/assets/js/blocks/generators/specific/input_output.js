...
Blockly.Python.io_smt32_getSwitchState = function (block) {
    const sw = block.getFieldValue("SWITCH");
    const switchName = sw.toLowerCase();
    Blockly.Python.addImport('pyb', IMPORT_PYB);
    Blockly.Python.addInit(switchName, switchName + " = pyb.Pin(\'" + sw + "\')");
    Blockly.Python.addInit(switchName + '_init', switchName + ".init(pyb.Pin.IN, pyb.Pin.PULL_UP, af=-1)");
    return [switchName + ".value()", Blockly.Python.ORDER_ATOMIC];
};
...
Blockly.Python.io_stopPwm = function (block) {
    ...
    let pinTable= Blockly.Constants.Pins.PWM[Blockly.Constants.getSelectedBoard()];
    let pinName = "";
    for (let elt in Blockly.Constants.Pins.PWM[Blockly.Constants.getSelectedBoard()]){
        if (pinTable[elt].includes(pin)){
            pinName=pinTable[elt]+""
            pinName=pinName.split(',')[0]
            break;
        }
    }
    return "pwm_" + pinName.toLowerCase()+".pulse_width_percent(0)" + NEWLINE;
};
...
Blockly.Python.io_getKeypadNumber = function (block) {
    ...
    Blockly.Python.addFunction('getKeypadNumber', FUNCTIONS_WB55.DEF_KEYPAD_GET_NUMBER);
