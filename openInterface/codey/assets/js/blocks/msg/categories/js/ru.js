/**
 * @fileoverview French translation file for Codey Toolbox. (FR)
 */

// Toolbox category names.
Blockly.Msg['CATEGORY_COMMUNICATION'] = 'Communication';
Blockly.Msg['CATEGORY_SENSORS'] = 'Capteurs';
Blockly.Msg['CATEGORY_ROBOTS'] = 'Robots';
Blockly.Msg['CATEGORY_NETWORK'] = 'Réseau';
Blockly.Msg['CATEGORY_TEXT'] = 'Texte';
Blockly.Msg['CATEGORY_VARIABLES'] = 'Variables';
Blockly.Msg['CATEGORY_LISTS'] = 'Listes';
Blockly.Msg['CATEGORY_PROCEDURES'] = 'Fonctions';
Blockly.Msg['CATEGORY_EXCEPTION'] = 'Exceptions';
Blockly.Msg['CATEGORY_COLOUR'] = 'Couleur';
// Category names in 'vittascience' mode.
Blockly.Msg['CATEGORY_DISPLAY'] = 'Affichage';
Blockly.Msg['CATEGORY_IO'] = 'Entrées/Sorties';
Blockly.Msg['CATEGORY_LOOPS'] = 'Boucles';
Blockly.Msg['CATEGORY_ACTUATORS'] = 'Musique';
Blockly.Msg['CATEGORY_LOGIC'] = 'Logique';
Blockly.Msg['CATEGORY_MATH'] = 'Math';
// Category names in 'scratch' mode.
Blockly.Msg['CATEGORY_APPEARANCE'] = 'Apparence';
Blockly.Msg['CATEGORY_SOUND'] = 'Son';
Blockly.Msg['CATEGORY_EVENTS'] = 'Événements';
Blockly.Msg['CATEGORY_CONTROL'] = 'Contrôle';
Blockly.Msg['CATEGORY_OPERATORS'] = 'Opérateurs';
// Toolbox subcategory labels.
Blockly.Msg['SUBCATEGORY_TIME'] = 'Temps';
Blockly.Msg['SUBCATEGORY_BUTTONS'] = 'Boutons';
Blockly.Msg['SUBCATEGORY_OTHER'] = 'Autres';
Blockly.Msg['SUBCATEGORY_MOTION'] = 'Mouvement';
Blockly.Msg['SUBCATEGORY_ROCKY_MOTORS'] = 'Moteurs';
Blockly.Msg['SUBCATEGORY_ROCKY_SENSORS'] = 'Capteurs';
Blockly.Msg['CATEGORY_COMMUNICATION'] = 'Связь';
Blockly.Msg['CATEGORY_SENSORS'] = 'Датчики';
Blockly.Msg['CATEGORY_ROBOTS'] = 'Роботы';
Blockly.Msg['CATEGORY_NETWORK'] = 'Сеть';
Blockly.Msg['CATEGORY_TEXT'] = 'Текст';
Blockly.Msg['CATEGORY_VARIABLES'] = 'Переменные';
Blockly.Msg['CATEGORY_LISTS'] = 'Списки';
Blockly.Msg['CATEGORY_PROCEDURES'] = 'Функции';
Blockly.Msg['CATEGORY_EXCEPTION'] = 'Исключения';
Blockly.Msg['CATEGORY_COLOUR'] = 'Цвет';
Blockly.Msg['CATEGORY_DISPLAY'] = 'Экран';
Blockly.Msg['CATEGORY_IO'] = 'Blockly.Msg[\'CATEGORY_DISPLAY\'] = \'Отображение\';';
Blockly.Msg['CATEGORY_LOOPS'] = 'Циклы';
Blockly.Msg['CATEGORY_ACTUATORS'] = 'Музыка';
Blockly.Msg['CATEGORY_LOGIC'] = 'Логика';
Blockly.Msg['CATEGORY_MATH'] = 'Blockly.Msg[\'CATEGORY_ACTUATORS\'] = \'Актуаторы\';';
Blockly.Msg['CATEGORY_APPEARANCE'] = 'Внешний вид';
Blockly.Msg['CATEGORY_SOUND'] = 'Звук';
Blockly.Msg['CATEGORY_EVENTS'] = 'События';
Blockly.Msg['CATEGORY_CONTROL'] = 'Управление';
Blockly.Msg['CATEGORY_OPERATORS'] = 'Операторы';
Blockly.Msg['SUBCATEGORY_TIME'] = 'Время';
Blockly.Msg['SUBCATEGORY_BUTTONS'] = 'Кнопки';
Blockly.Msg['SUBCATEGORY_OTHER'] = 'Прочее';
Blockly.Msg['SUBCATEGORY_MOTION'] = 'Движение';
Blockly.Msg['SUBCATEGORY_ROCKY_MOTORS'] = 'Моторы';
Blockly.Msg['SUBCATEGORY_ROCKY_SENSORS'] = 'Датчики';
