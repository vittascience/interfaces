 ...
 * @description this awesome shader was taken from https://discourse.threejs.org/t/starry-shader-for-sky-sphere/7578
 * */

/**
 * Background class
 * @class
 * @classdesc This class is used to create the background of the scene
...
export default class Background {
	...
	init() {
		...
		var skyDome = new THREE.Mesh(sphereGeometry, this.starMaterial);
    skyDome.name = 'backgroundSkyDome';
		this.experience.scene.add(skyDome);

		const floor = new THREE.Mesh(new THREE.PlaneGeometry(1000, 1000), new THREE.MeshStandardMaterial({ color: 0x8dbacc }));
	...
	update() {
		const animateStars = () => {
			const deltaTime = this.experience.clock.getDelta();
			this.starMaterial.uniforms.time.value += deltaTime * 1000; // Update time for animation
		};

		this.experience.movementObjectFunctions.animateStars = animateStars.bind(this);
	}
