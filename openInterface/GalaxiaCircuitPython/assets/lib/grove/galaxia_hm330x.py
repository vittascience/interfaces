import time

HM330_I2C_ADDR = 0x40
HM330_INIT = 0x80
HM330_MEM_ADDR = 0x88

class HM330X:

  def __init__(self, i2c, addr=HM330_I2C_ADDR):
    if i2c == None:
      raise ValueError("I2C object 'HM330X' needed as argument!")
    self._i2c = i2c
    self._i2c.try_lock()
    i2cModules = self._i2c.scan()
    if addr not in i2cModules:
      error = "Unable to find module 'HM330X' at address " + str(hex(addr)) + ". Please check connections with the board.\n"
      error += "[Info] I2C address.es detected: " + str([hex(a) for a in i2cModules])
      print(error)
      raise ValueError(error)
    self._addr = addr
    self._write([HM330_INIT])

  def read_data(self):
    self._i2c.writeto(self._addr, bytes([HM330_MEM_ADDR]))
    crc_result = bytearray(29)
    self._i2c.readfrom_into(self._addr, crc_result)
    return crc_result

  def _write(self, buffer):
    self._i2c.writeto(self._addr, bytearray(buffer))

  def check_crc(self, data):
    sum=0
    for i in range(29-1):
      sum+=data[i]
    sum=sum&0xff
    return (sum==data[28])

  def parse_data(self, data):
    std_PM1=data[4]<<8|data[5]
    std_PM2_5=data[6]<<8|data[7]
    std_PM10=data[8]<<8|data[9]
    atm_PM1=data[10]<<8|data[11]          
    atm_PM2_5=data[12]<<8|data[13]
    atm_PM10=data[14]<<8|data[15]
    return [std_PM1,std_PM2_5,std_PM10,atm_PM1,atm_PM2_5,atm_PM10]

  def getData(self, select):
    datas=self.read_data()
    time.sleep(0.005)
    if (self.check_crc(datas)==True):
      data_parsed=self.parse_data(datas)
      return data_parsed[select]