...
class TH02(object):
  ...
  def init_humidity(self):
    ...
    self._i2c.writeto(self._addr, bytes([self.REGISTER_CONFIG, self.CONVERSION_HUMIDITY]))

  def is_ready(self):
    """Is sensor done with conversion"""
    self._i2c.writeto(self._addr, bytes([self.REGISTER_STATUS]))
    status = bytearray(1)
    self._i2c.readfrom_into(self._addr, status)
    # Extract ready bit
    return bool(not (status & 0x01))
  ...
  def read_data(self):
    ...
    self._i2c.writeto(self._addr, bytes([self.REGISTER_DATAH]))
    crc_result = bytearray(2)
    self._i2c.readfrom_into(self._addr, crc_result)
    return crc_result[0] << 8 | crc_result[1]
  ...
  def get_temperature(self):
    ...
    self._i2c.writeto(self._addr, bytes([self.REGISTER_CONFIG, self.CONVERSION_TEMP]))
