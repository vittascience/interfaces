...
#
# adaptation pour la Galaxia à partir
# src: https://github.com/itechnofrance/micropython/blob/master/librairies/paj7620/paj7620.py
# modified by: Léo Meillier (leomlr) 2022
...
class PAJ7620():
  def __init__(self, i2c, addr=PAJ7620_DEFAULT_I2C_ADDR):
    if i2c == None:
      raise ValueError("I2C object 'PAJ7620' needed as argument!")
    self._i2c = i2c
    self._i2c.try_lock()
    i2cModules = self._i2c.scan()
    if addr not in i2cModules:
      error = "Unable to find module 'PAJ7620' at address " + str(hex(addr)) + ". Please check connections with the board.\n"
  ...
  def paj7620WriteReg(self, addr, cmd):
    ...
    self._i2c.writeto_mem(self._addr, bytes([addr, cmd]))
  ...
  def paj7620ReadReg(self, addr, size):
    # Lecture d'un bloc a l'adresse 'addr' avec un nombre d'octets 'size'
