...
class SHT31(object):
  ...
  def __init__(self, i2c, addr=SHT31_DEFAULT_I2C_ADDR):
    """
    Initialize a sensor object on the given I2C bus and accessed by the
    given address.
    """
    if i2c == None:
      raise ValueError("I2C object 'SHT31' needed as argument!")
    self._i2c = i2c
    self._i2c.try_lock()
    i2cModules = self._i2c.scan()
    if addr not in i2cModules:
      error = "Unable to find module 'SHT31' at address " + str(hex(addr)) + ". Please check connections with the board.\n"
