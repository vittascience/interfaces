...
class SCD30:
  ...
  def start_continous_measurement(self, ambient_pressure=0):
    ...
    data = bint + bytes([crc])
    self.__write_command(self.START_CONT_MEASURE)
  ...
  def set_measurement_interval(self, interval):
    ...
    crc = self.__crc(bint[0], bint[1])
    data = bint + bytes([crc])
    self.__write_command(self.SET_MEASURE_INTERVAL)
    self._i2c.writeto(self.addr, data)
  ...
  def set_automatic_recalibration(self, enable):
    ...
    crc = self.__crc(bint[0], bint[1])
    data = bint + bytes([crc])
    self.__write_command(self.SET_FRC)
    self._i2c.writeto(self.addr, data)
  ...
  def set_temperature_offset(self, offset):
    ...
    crc = self.__crc(bint[0], bint[1])
    data = bint + bytes([crc])
    self.__write_command(self.SET_TEMP_OFFSET)
    self._i2c.writeto(self.addr, data)
  ...
  def set_altitude_comp(self, altitude):
    ...
    crc = self.__crc(bint[0], bint[1])
    data = bint + bytes([crc])
    self.__write_command(self.SET_ALT_COMP)
    self._i2c.writeto(self.addr, data)
  ...
  def __read_bytes(self, cmd, count):
    ...
    time.sleep(self.pause/1000000)
    crc_result = bytearray(count)
