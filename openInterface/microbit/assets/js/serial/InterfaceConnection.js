const InterfaceConnection = {
	...
	*/
	...
	webusb: {
		...
		/**
		 * Allow to switch Micropython firmware for micro:bit V2 in filsystem.
		 * @param {string} hexFile 
		 */
		changeFirmware: async function (hexFile) {
			await InterfaceConnection.doDisconnect();
			InterfaceConnection.fsWrapper = new microbitFsWrapper('main.py', hexFile);
			await InterfaceConnection.fsWrapper.initialize();
			await this.init(InterfaceConnection.fsWrapper);
		},
		...
		flashProgram: async function () {
			...
			if (!this.isUploading) {
				...
				const superbitRegex = /(import|from) (superbit|WOM_Sensor_Kit)( import|)/;
				if (superbitRegex.test(code) && !this.superbitInitialized) {
					this.superbitInitialized = true;
					await this.changeFirmware('SuperbitPython_Library_V2.hex');
				} else if (!superbitRegex.test(code) && this.superbitInitialized) {
					this.superbitInitialized = false;
					await this.changeFirmware('micropython-microbit-v2.1.1.hex');
				}
