const $builtinmodule = function () {
	...
	mod._data.octave = 4;
	mod._data.audioCtx = null;

	const set_tempo = function (ticks, bpm) {
		if (ticks === undefined)
	...
	const play = function (music, pin, wait, loop) {
		let module = Simulator.getModuleByKey(`mb-buzzer`);
		let id = module.id;
		if (!Simulator.code.match(module.regex)) {
		...
		return Simulator.runAsync(function (resolve, reject) {
			...
			function playNextNote() {
				...
				if (!mod._data.audioCtx) {
					...
					Simulator.audioContext = mod._data.audioCtx;
				}
				if (mod._data.osc) {
					mod._data.osc.stop();
					Simulator.setAnimator(module, id, 0);
				...
				if (n.f > 0) {
					Simulator.setAnimator(module, id, n.f);
	...
	play.$defaults = [undefined, undefined, new Sk.builtin.bool(true), new Sk.builtin.bool(false)];
	play.co_numargs = 4;
	mod.play = new Sk.builtin.func(play);

	const pitch = function (frequency, duration, pin, wait) {
		let module = Simulator.getModuleByKey(`mb-buzzer`);
		let id = module.id;
		if (!Simulator.code.match(module.regex)) {
			module = Simulator.getModuleByKey('buzzer');
	...
	pitch.co_varnames = ['frequency', 'duration', 'pin', 'wait'];
	pitch.$defaults = [undefined, new Sk.builtin.int_(-1), undefined, new Sk.builtin.bool(false)];
	pitch.co_numargs = 4;
	...
	mod.DADADADUM = Sk.ffi.remapToPy([
		"r4:2", "g", "g", "g", "eb:8", "r:2", "f", "f", "f", "d:8"
	]);

	mod.ENTERTAINER = Sk.ffi.remapToPy([
		"d4:1", "d#", "e", "c5:2", "e4:1", "c5:2", "e4:1", "c5:3",
		"c:1", "d", "d#", "e", "c", "d", "e:2", "b4:1", "d5:2", "c:4"
	]);
	mod.PRELUDE = Sk.ffi.remapToPy([
	...
	mod.WAWAWAWAA = Sk.ffi.remapToPy([
