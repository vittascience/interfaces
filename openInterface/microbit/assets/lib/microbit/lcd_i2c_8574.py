# Implements a HD44780 character LCD connected via PCF8574 on I2C in Micropython.
...
class I2cLcd:
  ...
  def __init__(self, i2c_addr=0x27, dim=(16, 2), scroll=True):  # default address of PCF8574 is 0x27
    self.i2c = i2c
    ...
    self.backl = 0x08
    self.i2c.write(self.i2c_addr, bytearray([0]))      # Init I2C
    sleep_us(20000)                       # Allow LCD time to powerup
    for _ in range(3):                     # Send reset 3 times
      self.i2c.write(self.i2c_addr, bytearray((0x34, 0x30))) # LCD_FUNCTION_RESET
      sleep_us(5000)                      # Need to delay at least 4.1 msec
    self.i2c.write(self.i2c_addr, bytearray((0x24, 0x20))) # LCD_FUNCTION, put LCD into 4 bit mode
    sleep_us(1000)
  ...
  def set_display(self, on=True, backl=None):
    ...
    if backl is not None:
      ...
      self.i2c.write(self.i2c_addr, bytearray((self.backl,)))
  ...
  def _wr(self, data, dbit=0):
    ...
    self.i2c.write(self.i2c_addr, bytearray((b0 | 0x04, b0, b1 | 0x04, b1)))
