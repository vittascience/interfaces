...
class WUKONG(object):
  ...
  def set_servo(self, servo, angle, servoType = _180):
    """基本描述
    ...
    if servo > 7 or servo < 0:
      raise ValueError('Servomotor S' + str(servo) + ' is invalid.')

    if servoType == self._180:
      angle = map_value(angle, 0, 180, 0, 180)
    elif servoType == self._270:
      angle = map_value(angle, 0, 270, 0, 180)
    elif servoType == self._360:
      angle = map_value(angle, 0, 360, 0, 180)
    else:
      raise ValueError('Servo type \'' + str(type) + '\' is invalid.')
  ...
  def set_servo_speed(self, servo, speed):
    angle = map_value(speed, -100, 100, 0, 180)
