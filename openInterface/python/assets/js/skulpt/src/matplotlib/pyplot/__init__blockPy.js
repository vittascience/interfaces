/**
 * src: https://github.com/blockpy-edu/skulpt/blob/master/src/lib/matplotlib/pyplot/__init__.js
 * @Author acbart
 * @modified by Léo Meillier (leo mlr)
 * 
 * Note: Have to use https://d3js.org/d3.v5.min.js (version 5)
 * This library is adadpted for Skulpt v1.2
 */
...
function matplotlib_pyplot_mod(mod, import_module) {
    ...
    const CLASS_NDARRAY = "numpy.ndarray";

    if (Sk.canvas === undefined) {
        throw new Sk.builtin.NameError("Can not resolve drawing area. Sk.canvas is undefined!");
    }
    const getCanvasId = function () {
        return "#" + Sk.canvas;
    };

    const formatNumpyArray = function (ndarray) {
        const data = Sk.ffi.unwrapn(ndarray);
        var dim = 0;
        ...
        return data.buffer.slice(0, dim).map(function (x) { return Sk.ffi.remapToJs(x); });
    };
    ...
    function makeChart() {
        ...
        return {
            ...
            margin: margin,
            width: $(getCanvasId()).width() - margin.left - margin.right,
            height: $(getCanvasId()).height() - margin.top - margin.bottom,
    ...
    let plot_f = function (kwa) {
        ...
        let chart = getChart();
        if (Sk.abstr.typeName(args[0]) === CLASS_NDARRAY) {
            args[0] = new Sk.builtin.list(formatNumpyArray(args[0]));
        }
        if (Sk.abstr.typeName(args[1]) === CLASS_NDARRAY) {
            args[1] = new Sk.builtin.list(formatNumpyArray(args[1]));
        }
    ...
    function attachCanvasToChart(chart, yAxisBuffer) {
        chart.svg = d3.select(getCanvasId()).append("div").append("svg");
        chart.svg.attr("class", "chart");
        chart.svg.attr("width", $(getCanvasId()).width());
        chart.svg.attr("height", $(getCanvasId()).height());
    ...
    function finalizeChart(chart) {
        ...
        img.onload = function () {
            ...
            canvas.width = $(getCanvasId()).width();
            canvas.height = $(getCanvasId()).height();
