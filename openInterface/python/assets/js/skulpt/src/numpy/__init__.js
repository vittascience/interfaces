...
var $builtinmodule = function (name) {
    ...
    **/
     ...
     */
    const NPY_MINIMUM_PRECISION = 1e-15;
    var current_longests = null;
    var is_number_array = false;
    /**
    ...
    */
    ...
    function PyArray_GetPriority(obj, default_) {
        ...
        ret = Sk.builtin.getattr(obj, new Sk.builtin.str('__array_priority__'), Sk.builtin.none());
    ...
    function PyArray_FromAny(op, dtype, min_depth, max_depth, requirements, context) {
        ...
        if (PyArray_Check(op)) {
            ...
            unpack(op, elements, state, dtype);
        }
        ...
        // check for min_depth
        const ndmin = Sk.builtin.asnum$(min_depth);
        ...
        const _shape = new Sk.builtin.tuple(state.shape.map(function (x) {
            return new Sk.builtin.int_(x);
        }));

        const _buffer = new Sk.builtin.list(elements);
    ...
    function dump_data(strPtr, nPtr, max_n, data, nd, dimensions, strides, self) {
        ...
        if (nd === 0) {
             ...
             */
            let isInt = false;
            if (!Sk.builtin.checkNone(self.v.dtype)) {
                if (Sk.builtin.checkInt(new self.v.dtype(1))) {
                    op = new Sk.builtin.int_(Math.ceil(op.v));
                    sp = new Sk.builtin.str(op.v);
                } else if (Sk.builtin.checkFloat(new self.v.dtype(1))) {
                    op = numpyRound(op, N);
                    sp = new Sk.builtin.str(op.v);
                    if (op.v === Math.round(op.v)) {
                        isInt = true;
                    }
                }
            }
    ...
    function numpyRound(number, len) {
        if (Sk.builtin.checkFloat(number)) {
            const float = number.v;
            const Intlen = new Sk.builtin.str(Math.round(float));
            const prevLen = len - 1;
            const decLen = prevLen - Intlen.v.length - 1;
            if (decLen >= 0) {
                const floatMinusOneDecimal = toFixedTrunc(float, decLen);
                if (Math.abs(float - floatMinusOneDecimal) < NPY_MINIMUM_PRECISION) {
                    return numpyRound(new Sk.builtin.float_(floatMinusOneDecimal), prevLen);
                }
            }
            return number;
        } else {
    ...
    function formatOneDimArray(elements, isNumber = true) {
        if (elements.length > 0) {
            elements = elements.map((e) => e.replace('[', '').replace(']', ''));
            if (isNumber) {
                for (var i = 0; i < elements.length; i++) {
                    const diff = current_longests.len - elements[i].length;
                    const intLen = current_longests.intLen - String(parseInt(elements[i])).length;
                    const after = diff - intLen;
                    elements[i] = ' '.repeat(intLen && intLen >= 0 ? intLen : 0) + elements[i] + ' '.repeat(after && after >= 0 ? after : 0);
                }
            }
    ...
    function formatArraySpaces(string, dim, sourceDim, last = false) {
        if (!sourceDim) sourceDim = dim;
        const matrix = string.split(", " + '['.repeat(dim - 1));
        if (dim > 1) {
            let newStr = "[" + formatArraySpaces(matrix[0].substr(1), dim - 1, sourceDim) + '\n';
            const lastIndex = matrix.length - 1;
            for (var i = 1; i < lastIndex; i++) {
                newStr += ' '.repeat(sourceDim - dim + 1) + formatArraySpaces('[' + matrix[i], dim - 1, sourceDim) + '\n';
            }
            const lastArr = matrix[lastIndex].substr(0, matrix[lastIndex].length - 1);
            newStr += ' '.repeat(sourceDim - dim + 1) + formatArraySpaces('[' + lastArr, dim - 1, sourceDim, true) + ']';
            return last ? newStr : newStr + '\n'.repeat(dim - 1);
        } else if (dim == 1) {
            return formatOneDimArray(matrix, is_number_array);
        } else {
            return string;
        }
    }

    function getLongest(data, dtype) {
        let array = Sk.ffi.remapToJs(new Sk.builtin.list(data));
        if (!Sk.builtin.checkNone(dtype)) {
            if (Sk.builtin.checkInt(new dtype(1))) {
                array = array.map(e => new Sk.builtin.int_(Math.trunc(e)));
            }
            else if (Sk.builtin.checkFloat(new dtype(1))) {
                array = data.map(e => {
                    const str = new Sk.builtin.str(e.v);
                    return numpyRound(e, str.v.length);
                });
            }
        }
        const stringArray = array.map(e => String(e.v));
        const longestItem = stringArray.sort((a, b) => b.length - a.length)[0];
        const longestItemRounded = String(Math.round(parseFloat(longestItem)));
        const roundLongestItem = Math.max(...array.map(e => String(Math.round(e.v)).length));
        return {
            "len": longestItem.length + roundLongestItem - longestItemRounded.length,
            "intLen": roundLongestItem
        };
    }
    /**
     * END VITTASCIENCE ADDS
    ...
    function array_repr_builtin(self, repr) {
        ...
        is_number_array = !Sk.builtin.checkNone(self.v.dtype) && (self.v.dtype == Sk.builtin.float_ || self.v.dtype == Sk.builtin.int_);
        if (is_number_array) {
            current_longests = getLongest(PyArray_DATA(self), self.v.dtype);
        }
        const npString = formatArraySpaces(strPtr.str, PyArray_NDIM(self), PyArray_NDIM(self));
        string = new Sk.builtin.str(npString);
        if (repr) {
     ...
     **/
    function unpack(py_obj, buffer, state, dtype) {
        ...
        if (py_obj instanceof Sk.builtin.list || py_obj instanceof Sk.builtin.tuple) {
            const py_items = remapToJs_shallow(py_obj);
            state.level += 1;
            if (state.level > state.shape.length) {
                state.shape.push(py_items.length);
            } else {
                if (dtype !== 'object' && py_items.length !== state.shape[state.level - 1]) {
                    throw new Sk.builtin.ValueError("Creating an ndarray from ragged nested sequences (which is a list-or-tuple of lists-or-tuples-or ndarrays with different lengths or shapes) is deprecated. If you meant to do this, you must specify 'dtype=object' when creating the ndarray.")
                }
            }
            for (var i = 0; i < py_items.length; i++) {
                unpack(py_items[i], buffer, state, dtype);
            }
            state.level -= 1;
        } else {
            buffer.push(py_obj);
    ...
    **/
    var ndarray_f = function ($gbl, $loc) {
        $loc.__init__ = new Sk.builtin.func(function (self, shape, dtype, buffer,
            ...
            ndarrayJs.dtype = dtype || Sk.builtin.none();
        ...
        **/
        ...
        $loc.__repr__ = new Sk.builtin.func(function (self) {
            return array_repr_builtin(self, 0);
        });
    ...
    mod[CLASS_NDARRAY] = new Sk.misceval.buildClass(mod, ndarray_f,
        CLASS_NDARRAY, []);
    ...
    **/
    ...
    var linspace_f = function (start, stop, num, endpoint, retstep) {
        ...
        for (i = 0; i < samples.length; i++) {
            samples[i] = new Sk.builtin.float_(samples[i]);
        }

        var buffer = new Sk.builtin.list(samples);
        var shape = new Sk.builtin.tuple([samples.length]);
        var ndarray = Sk.misceval.callsim(mod[CLASS_NDARRAY], shape, dtype, buffer);
    ...
    var arange_f = function (start, stop, step, dtype) {
        ...
        // set to float
        if (!dtype || dtype == Sk.builtin.none()) {
    ...
    arange_f.$defaults = [0, 1, 1, Sk.builtin.none()];
    ...
    var array_f = function (object, dtype, copy, order, subok, ndmin) {
        ...
        return PyArray_FromAny(object, dtype, ndmin);
    };

    array_f.co_varnames = ['object', 'dtype', 'copy', 'order',
        'subok', 'ndmin'
    ];
    array_f.$defaults = [null, Sk.builtin.none(), true, new Sk.builtin.str(
    ...
    asanyarray_f.$defaults = [null, Sk.builtin.none(), Sk.builtin.none()];
    ...
    **/
    ...
    zeros_f.$defaults = [new Sk.builtin.tuple([]), Sk.builtin.none(), new Sk.builtin.str('C')];
    ...
    **/
    ...
    full_f.$defaults = [
        new Sk.builtin.tuple([]), Sk.builtin.none(), Sk.builtin.none(), new Sk
            .builtin
            .str('C')
    ];
    ...
    prod_f.$defaults = [null, Sk.builtin.none(), Sk.builtin.none(), Sk.builtin.none(), Sk.builtin.bool.false$];
    mod.prod = new Sk.builtin.func(prod_f);

    var sum_f = function (a, axis, dtype, out, keepdims, initial, where) {
        ...
        if (initial != null && initial != Sk.builtin.bool.false$) {
            throw new Sk.builtin.NotImplementedError("the 'initial' parameter is currently not supported");
        }

        if (where != null && where != Sk.builtin.bool.false$) {
            throw new Sk.builtin.NotImplementedError("the 'where' parameter is currently not supported");
        }
        ...
        const _buffer = PyArray_DATA(a);

        let sum = new Sk.builtin.float_(0.0);
        for (var i = 0; i < PyArray_SIZE(a); i++) {
    ...
    sum_f.co_varnames = ['a', 'axis', 'dtype', 'out', 'keepdims', 'initial', 'where'];
    sum_f.$defaults = [Sk.builtin.none(), Sk.builtin.none(), Sk.builtin.none(), Sk.builtin.bool.false$, Sk.builtin.bool.false$, Sk.builtin.bool.false$];
    mod.sum = new Sk.builtin.func(sum_f);

    var average_f = function (a, axis, weights, returned, keepdims) {

        if (axis != null && !Sk.builtin.checkNone(axis)) {
            throw new Sk.builtin.NotImplementedError("the 'axis' parameter is currently not supported");
        }

        if (returned != null && returned != Sk.builtin.bool.false$) {
            throw new Sk.builtin.NotImplementedError("the 'returned' parameter is currently not supported");
        }
        ...
        if (Sk.builtin.checkNone(weights)) {
            return mean_f(a, axis, Sk.builtin.none());
        } else {
            if (!PyArray_Check(weights)) {
                if (Sk.builtin.checkIterable(a)) {
                    weights = PyArray_FromAny(weights);
                } else {
                    throw new Sk.builtin.ValueError('[Numpy] average() - weights parameter is not iterable');
                }
            }
        }
        ...
        if (!PyArray_Check(a)) {
            if (Sk.builtin.checkIterable(a)) {
                ...
                return new Sk.builtin.float_(Sk.builtin.abs(a));
            }
        }

        if (!arrayEquals(a.v.shape, weights.v.shape)) {
            throw new Sk.builtin.ValueError('[Numpy] average() - array shape and its weights shape are not same');
        }

        _buffer = PyArray_DATA(a);
        _weights_buffer = PyArray_DATA(weights);

        _weights_sum = sum_f(weights);

        let sum = new Sk.builtin.float_(0.0);
        for (var i = 0; i < PyArray_SIZE(a); i++) {
            const weighted = Sk.abstr.numberBinOp(_weights_buffer[i], _buffer[i], 'Mult');
            sum = Sk.abstr.numberBinOp(sum, weighted, 'Add');
        }

        return new Sk.builtin.float_(sum.v / _weights_sum.v);
    };
    average_f.co_varnames = ['a', 'axis', 'weights', 'returned', 'keepdims'];
    average_f.$defaults = [Sk.builtin.none(), Sk.builtin.none(), Sk.builtin.bool.false$, Sk.builtin.bool.false$];
    mod.average = new Sk.builtin.func(average_f);

    var mean_f = function (a, axis, dtype, out, keepdims) {
        ...
        sum = sum_f(a, axis, dtype, out, keepdims);

        // call PyArray_Return
        return new Sk.builtin.float_(sum.v / PyArray_SIZE(a));
    };
    mean_f.co_varnames = ['a', 'axis', 'dtype', 'out', 'keepdims'];
    mean_f.$defaults = [null, Sk.builtin.none(), Sk.builtin.none(), Sk.builtin.none(), Sk.builtin.bool.false$];
    mod.mean = new Sk.builtin.func(mean_f);

    var std_f = function (a, axis, dtype, out, ddof, keepdims, where) {
        ...
        const mean = mean_f(a, axis, dtype, out, keepdims);

        let _buffer = PyArray_DATA(a);
        for (var i = 0; i < PyArray_SIZE(a); i++) {
            _buffer[i] = Sk.abstr.numberBinOp(_buffer[i], mean, 'Sub');
            _buffer[i] = Sk.abstr.numberBinOp(_buffer[i], new Sk.builtin.int_(2), 'Pow');
        }
        ...
        return new Sk.builtin.float_(Math.sqrt(sum.v / _buffer.length));
    };
    std_f.co_varnames = ['a', 'axis', 'dtype', 'out', 'ddof', 'keepdims', 'where'];
    std_f.$defaults = [Sk.builtin.none(), Sk.builtin.none(), Sk.builtin.none(), new Sk.builtin.int_(0), Sk.builtin.bool.false$, Sk.builtin.bool.false$];
    ...
    **/
    ...
    ones_f.$defaults = [
        new Sk.builtin.tuple([]), Sk.builtin.none(), new Sk.builtin.str('C')
    ];
    ...
    **/
    ...
    dot_f.$defaults = [Sk.builtin.none(),
    ...
    var any_f = function (a, axis, out, keepdims) {
        ...
        if (axis != undefined && !Sk.builtin.checkNone(axis)) {
            throw new Sk.builtin.ValueError('"axis" parameter not supported');
        }
    ...
    any_f.$defaults = [Sk.builtin.none(),
    Sk.builtin.none(), Sk.builtin.bool.false$
    ];
    ...
    all_f.$defaults = [Sk.builtin.none(),
    Sk.builtin.none(), Sk.builtin.bool.false$
    ];
    ...
    less_f.$defaults = [Sk.builtin.none(),
    Sk.builtin.none(), Sk.builtin.bool.false$
    ];
    ...
    less_equal_f.$defaults = [Sk.builtin.none(),
    Sk.builtin.none(), Sk.builtin.bool.false$
    ];
    ...
    greater_f.$defaults = [Sk.builtin.none(),
    Sk.builtin.none(), Sk.builtin.bool.false$
    ];
    ...
    greater_equal_f.$defaults = [Sk.builtin.none(),
    Sk.builtin.none(), Sk.builtin.bool.false$
    ];
    ...
    equal_f.$defaults = [Sk.builtin.none(),
    Sk.builtin.none(), Sk.builtin.bool.false$
    ];
    ...
    not_equal_f.$defaults = [Sk.builtin.none(),
    Sk.builtin.none(), Sk.builtin.bool.false$
    ];
