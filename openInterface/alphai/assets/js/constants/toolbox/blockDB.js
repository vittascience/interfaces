/**
 * Database of defaut blocks in AlphAI toolbox.
...
const TOOLBOXES_BLOCKS_CONTENT = {
    ...
    get: function () {
        return {
            /** AlphAI default blocks */

            // robot
            "robot_printMessage": this.Set.text('MESSAGE', '{hello}'),
            "robot_setDirection": this.Set.number("SPEED", 20) + '<mutation duration="false"></mutation>',
            "robot_setMotor": this.Set.number("MOTOR_LEFT", 20) + this.Set.number("MOTOR_RIGHT", 20) + '<mutation duration="false"></mutation>',
            "robot_setBuzzer": '<mutation duration="false"></mutation>',
            "robot_setLedsRGB": this.Set.number("R", 255) + this.Set.number("G", 0) + this.Set.number("B", 0),
            "robot_setLedsPalette": this.Set.colour_picker(),

            // io - time
            "io_pause": this.Set.number("TIME", 1),
            "io_waitUntil": "<value name='UNTIL'><block type='logic_compare'>" + this.Set.field("OP", "EQ") + this.Set.number("B", 1) + "</block></value>",            // actuators
