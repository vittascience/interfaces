 ...
 * Source: https://github.com/cosinnes/calcMD5
 */
var hex_chr = "0123456789abcdef";
function rhex(i) {
    for (str = "", j = 0; j <= 3; j++) {
        str += hex_chr.charAt(i >> 8 * j + 4 & 15) + hex_chr.charAt(i >> 8 * j & 15);
    }
    return str;
}
function str2blks_MD5(d) {
    for (nblk = 1 + (d.length + 8 >> 6), blks = new Array(16 * nblk), i = 0; i < 16 * nblk; i++) {
        blks[i] = 0;
    }
    for (i = 0; i < d.length; i++) {
        blks[i >> 2] |= d.charCodeAt(i) << i % 4 * 8;
    }
...
function calcMD5(h) {
    for (x = str2blks_MD5(h), a = 1732584193, b = -271733879, c = -1732584194, d = 271733878, i = 0; i < x.length; i += 16) {
