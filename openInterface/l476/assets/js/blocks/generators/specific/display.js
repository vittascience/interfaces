/**
 * @fileoverview Display generators for L476.
 */
...
Blockly.Python.display_led_matrix_DrawBitmap = function (block) {
    ...
    // do not chain the draw method to avoid breaking the python traduction
...
Blockly.Python.display_l476_controlColorLed = function (block) {
    ...
    Blockly.Python.addInit('led', "led_green = pyb.LED(1)");
    if (inputBlock && (inputBlock.type == "io_digital_signal")) {
        return (state == '1' ? "led_green.on()" : "led_green.off()") + NEWLINE;
    } else {
        return "led_green.on() if " + state + " else " + "led_green.off()" + NEWLINE;
    }
};

Blockly.Python.display_l476_toggleColorLed = function (block) {
    Blockly.Python.addImport('pyb', IMPORT_PYB);
    Blockly.Python.addInit('led', "led_green = pyb.LED(1)");
    return "led_green.toggle()" + NEWLINE;
};
...
Blockly.Python.neopixel_codeInitialization = function (block, pin, ledCount) {
    ...
    Blockly.Python.addPowerOn('neopixel_' + numPin, "np_" + numPin + " = " + "neopixel." + "NeoPixel(" + numPin.replace('D', 'd') + ", " + ledCount + ")");
...
Blockly.Python.display_setClockGrove4Digit = function (block) {
    ...
    Blockly.Python.addFunction('getCurrentTime', FUNCTIONS_L476.DEF_GET_CURRENT_TIME);
    Blockly.Python.addInit(objName, objName + " = TM1637(clk=" + pinName_CLK + ", dio=" + pinName_DIO + ")");
...
Blockly.Python.display_setColorChainableRGBLed = function (block) {
    ...
    let red = Blockly.Python.valueToCode(block, "R", Blockly.Python.ORDER_ATOMIC);
    let green = Blockly.Python.valueToCode(block, "G", Blockly.Python.ORDER_ATOMIC);
    let blue = Blockly.Python.valueToCode(block, "B", Blockly.Python.ORDER_ATOMIC);
    if (red < 0) red = 0;
    if (red > 255) red = 255;
    if (green < 0) green = 0;
