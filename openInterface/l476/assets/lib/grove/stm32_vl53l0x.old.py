...
class VL53L0X(object):
  ...
  def __init__(self, i2c, addr=_VL53L0X_IIC_ADDR, io_timeout_s=0):
    ...
    if i2c == None:
      raise ValueError("I2C object 'VL53L0X' needed as argument!")
    self._i2c = i2c
    i2cModules = self._i2c.scan()
    if addr not in i2cModules:
      error = "Unable to find module 'VL53L0X' at address " + str(hex(addr)) + ". Please check connections with the board.\n"
