...
class NeoPixel:
  ...
  def __init__(self, pin, n, bpp=3, timing=1, brightness=0.15):
    self.pin = pin
    self.n = n
    self.bpp = bpp
  ...
  def __setitem__(self, i, v):
    ...
    for i in range(self.bpp):
      self.buf[offset + self.ORDER[i]] = round(v[i]*self.brightness)  # scale to brightness
  ...
  def fill(self, v):
    b = self.buf
    for i in range(self.bpp):
      c = round(v[i]*self.brightness)  # scale to brightness
