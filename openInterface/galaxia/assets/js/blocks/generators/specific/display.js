...
Blockly.Python.display_galaxia_led_set_colors_rgb = function (block) {
...
Blockly.Python.display_galaxia_led_set_colors_palette = function (block) {
    const color = Blockly.Python.valueToCode(block, "COLOR", Blockly.Python.ORDER_NONE) || "";
    if (color) {
        return "led.set_colors" + color + NEWLINE;
    }
    return "led.set_colors(0, 0, 0)" + NEWLINE;
};

Blockly.Python.display_galaxia_led_set_color = function (block) {
    const color = block.getFieldValue("COLOR");
    const value = Blockly.Python.valueToCode(block, "VALUE", Blockly.Python.ORDER_NONE) || 0;
    switch (color) {
        case 'RED':
            return "led.set_red(" + value + ")" + NEWLINE;
        case 'GREEN':
            return "led.set_green(" + value + ")" + NEWLINE;
        case 'BLUE':
            return "led.set_blue(" + value + ")" + NEWLINE;
    }
};

Blockly.Python.display_galaxia_led_get_color = function (block) {
    const color = block.getFieldValue("COLOR");
    switch (color) {
        case 'RED':
            return ["led.get_red()", Blockly.Python.ORDER_ATOMIC];
        case 'GREEN':
            return ["led.get_green()", Blockly.Python.ORDER_ATOMIC];
        case 'BLUE':
            return ["led.get_blue()", Blockly.Python.ORDER_ATOMIC];
    }
};
...
Blockly.Python.display_galaxia_raw_print_img = function (block) {
    ...
    return `display.raw.show()${NEWLINE}display.raw.print_bmp(${x}, ${y}, ${img})${NEWLINE}`;
};

Blockly.Python.display_galaxia_raw_print_custom_img = function (block) {
    const x = Blockly.Python.valueToCode(block, "X", Blockly.Python.ORDER_NONE) || 0;
    const y = Blockly.Python.valueToCode(block, "Y", Blockly.Python.ORDER_NONE) || 0;
...
Blockly.Python.display_lcdSetText = function (block) {
    ...
    if (addr == "0x3f" || addr == "0x27") {
        ...
        Blockly.Python.addInit('lcd8574', "lcd8574 = I2cLcd(I2C(scl=Pin(13), sda=Pin(14)), i2c_addr=" + addr + ")");
        if (Blockly.Constants.Utils.isInputTextBlock(block, "TEXT")) {
        ...
        Blockly.Python.addInit('lcd', "lcd = LCD1602(i2c=I2C(scl=Pin(13), sda=Pin(14)))");
        if (Blockly.Constants.Utils.isInputTextBlock(block, "TEXT")) {
...
Blockly.Python.display_setLEDintensity = function (block) {
    ...
    return "writePWM(" + pinName + ", " + value + "*1023/100)" + NEWLINE;
};
...
Blockly.Python.display_setVariableColorLED = function (block) {
    ...
    Blockly.Python.addFunction('writePWM', FUNCTIONS_GALAXIA.DEF_WRITE_WPM);
    return "writePWM(" + pinName + ", 1023*(1-" + value + "/100))" + NEWLINE;
};
