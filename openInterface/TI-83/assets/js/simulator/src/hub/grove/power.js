...
var $builtinmodule = function (name) {
    ...
    power.__name__ = new Sk.builtin.str('power');
	
	power.power = new Sk.misceval.buildClass(power, function($gbl, $loc) {

		power__init__ = function (self, pin) {
			if (pin !== undefined) {
                ...
                Simulator.Mosaic.specific.ti.showError("Il manque l'argument 'pin' à la fonction power()", "function takes 2 positional arguments but 1 were given.", 'TypeError');
			}
		};

		power__init__.co_varnames = ['self', 'pin'];
		power__init__.$defaults = [Sk.builtin.none()];

		$loc.__init__ = new Sk.builtin.func(power__init__);

		$loc.on = new Sk.builtin.func(function (self) {
			Simulator.setAnimator(module, module.id + '_' + self.pinName, 1023);
        ...
        $loc.set = new Sk.builtin.func(function (self, value) {
            if (value !== undefined) {
                if (Sk.builtin.checkInt(value)) {
                    Simulator.setAnimator(module, module.id + '_' + self.pinName, value.v / 100 * PWM_MAX_DUTY);
