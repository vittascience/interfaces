...
if(getLangInCookie() == 'fr' || /fr\.vittascience/.test(window.location.href)){
	WBBLANG['fr'] = CURLANG = {
		...
		vittaiframe: "Intégrer une interface Vittascience",
		...
		messages:{
			...
			image:{
				...
				imageNameInvalid: "Le nom du fichier n'est pas valide.<br>caractères autorisés: lettres sans accents, chiffres, -, _ et 1 seul point pour l'extension",
	...
	WBBLANG['en'] = CURLANG = {
		...
		messages:{
			...
			image:{
				...
				imageNameInvalid: "Filename invalid.<br>Allowed caracters: letters without accent, numbers, -, _ and only 1 period for the extension",
...
(function ($) {
		...
		this.options = {
			...
			allButtons: {
					...
					modal: {
						...
						tabs: [{
							input: [{
								...
								validation: '^http(s)?:\/\/([a-z]+\.)vittascience(.com)?'
							}]
						}]
					},
					transform: {
				...
				strike: {
					...
					buttonHTML: '<strike  style="font-size: 20px; margin:25% 25% !important;width: 100%;"><strong>S</strong></strike>',
					excmd: 'strikeThrough',
					transform: {
				...
				img: {
					title: CURLANG.img,
					buttonHTML: '<i class="fa fa-file-image" style="height:27px;width:31px;font-size:20px;text-align:center;padding-top: 5px;"></i>',
					hotkey: 'ctrl+shift+1',
					addWrap: true,
					modal: {
					...
					transform: {
						'<img src="{SRC}" class="img-fluid"/>': "[img]{SRC}[/img]",
				...
				numlist: {
					...
					transform: {
						'<ol>{SELTEXT}</ol>': "[list=1]{SELTEXT}[/list=1]",
						'<li>{SELTEXT}</li>': "[*]{SELTEXT}[/*]"
					}
				},
				quote: {
				...
				fontcolor: {
					...
					valueBBname: "color",
					subInsert: true,
					colors: "#000000,#3C3C3C,#5B5B5B,#8E8E8E,#ADADAD,#C5C5C5,#D3D2D3,#EDECED,#F2F2F2,#ffffff,-, \
							#ff9403,#448ae5,#7ac943,#9966ff,#ff4d6a,#87599d,#fc7417,#22b573,#3fa9f5,#1a6da8,#f9d142,#e58544,#0fb9b1,-,\
							#F4C5C6,#D8A79E,#FDE2C8,#FFF0C7,#D3E7CE,#C9DBDF,#C1D4F5,#C7DDF0,#D4CBE5,#E8CBD7, \
							#EB8F8F,#DC7362,#FAC594,#FFE293,#ABD2A0,#96BCC1,#98B9F0,#92BDE3,#AC9CCF,#D29CB4, \
							#E05C5D,#CA3A26,#F8AA65,#FFD564,#85BD75,#689BA5,#5F92E5,#609DD5,#846FB9,#BD6F95, \
							#CA0B10,#A01C0E,#E68739,#F1BB3A,#5A9F4A,#387582,#316BCF,#3078BC,#5D439A,#9F436D, \
							#88070B,#7D1E12,#AE5517,#B9861C,-,\
							#91080C,#FF6D1D,#FFFF32,#00FF2D,#00FFFE,#0C77C2,#0C00FA,#9300FA,#FF00FB,",
					transform: {
					...
					modal: function (cmd, opt, queryState) {
						...
						.html(`
							<div class="wbbm hastabs" hastabs"="" style="max-width: 500px; width: 600px; margin-top: 202.479px;">
								...
								<div class="wbbm-content" style="font-size:14px;">
										...
										<div class="tab-cont tab0" tid="0">													
												...
												<div class="fileupload" class="d-flex flex-column-reverse" style="text-align:center;margin:0 auto;">
													<div class="mb-3">
														...
														<p class="my-2">Format(s) : .jpg,.jpeg,.png,.svg,.webp,.gif,.apng ( 2Mb max.)</p>
						...
						this.$modal.find('#imageToUpload').change($.proxy(function (e) {
							...
							if (typeof imageToUpload === 'undefined') errors.push({errorType: 'imageIsRequired'})
								...
								const imageNameRegex = /^[a-zA-Z0-9_\-]+.[a-zA-Z0-9]+$/gi
								if(!imageToUpload.name.match(imageNameRegex)) errors.push({errorType: 'imageNameInvalid'})
								if(allowedExtensions.indexOf(imageExtension) === -1) errors.push({errorType: 'invalidImageExtension'})
								if(imageToUpload.size > 3000000) errors.push({errorType: 'imageSizeToLarge'})
							}
				...
				myfontsize: {
					...
					title: CURLANG.fontsize,
					excmd: 'fontSize',
					options: 'fs_16,fs_20,fs_24,fs_28,fs_32,fs_36,fs_40'
				},
				fs_16: {
					title: '16px',
					buttonText: "fs1",
					...
					exvalue: "1",
					transform: {
						'<font size="1">{SELTEXT}</font>': '[size=16]{SELTEXT}[/size]'
					}
				},
				fs_20: {
					title: '20px',
					...
					exvalue: "2",
					transform: {
						'<font size="2">{SELTEXT}</font>': '[size=20]{SELTEXT}[/size]'
					}
				},
				fs_24: {
					title: '24px',
					...
					exvalue: "3",
					transform: {
						'<font size="3">{SELTEXT}</font>': '[size=24]{SELTEXT}[/size]'
					}
				},
				fs_28: {
					title: '28px',
					...
					exvalue: "4",
					transform: {
						'<font size="4">{SELTEXT}</font>': '[size=28]{SELTEXT}[/size]'
					}
				},
				fs_32: {
					title: '32px',
					...
					exvalue: "5",
					transform: {
						'<font size="5">{SELTEXT}</font>': '[size=32]{SELTEXT}[/size]'
					}
				},
				fs_36: {
					title: '36px',
				...
				fs_40: {
					...
					exvalue: "7",
					transform: {
						'<font size="7">{SELTEXT}</font>': '[size=40]{SELTEXT}[/size]'
					}
				},
			},
			systr: {
	...
	$.wysibb.prototype = {
		...
		closeModal: function () {
			...
			this.$modal.find('#wbbm-submit').html(`${CURLANG.save}`)
			this.$modal.hide();
			this.lastRange = false;
			return this;
		},
