...
class Navigation extends Observable {
	...
	questionManager(nav) {
		...
		if (this.currentQuestionIndex === questionMaterialLength + questionUtilizationLength) {
			...
			this._model.saveImpactToLocalStorage();
		...
		if (this.currentQuestionIndex === questionMaterialLength + questionUtilizationLength + 1) {
			...
			const downloadPdfBtn = document.querySelector('#download-pdf');
			if (!downloadPdfBtn) this.addBtnDownloadPDF()
				else downloadPdfBtn.classList.remove('d-none');
	...
	updateNavButtons() {
		...
		const of= i18next.t("co2.nav.of")
		document.querySelector('.step').innerText = `${this.getCurrentStep()} ${of} ${total}`
