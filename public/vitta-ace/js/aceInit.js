...
const STM_TIPS_FR = {
    ...
    "pyb.I2C.mem_write(data, addr, memaddr, *, timeout=5000, addr_size=8)": "Ecrire dans la mémoire d'un périphérique I2C Paramètres : data (int ou buffer) -- nombre d'octets à écrire ou un tampon pour écrire dans addr (int) -- l'adresse du périphérique I2C memaddr (int) -- l'emplacement mémoire dans le délai d'expiration du périphérique I2C (int) le délai d'attente en millisecondes pour attendre l'écriture addr_size (int) -- largeur de memaddr : 8 ou 16 bits",
    ...
    "pyb.UART.writechar(char) ": " Ecrire un seul caractère sur le bus Paramètres : char (int) -- l'entier à écrire ",
...
function assignTips(doc) {
    ...
    var tips = null;
    if (doc.getElementById('iesp32')) {
        if (navLang == 'en' || navLang == 'en-US') {
            tips = MICROPYTHON_TIPS_EN;
        }
        else if (navLang == 'fr-FR' || navLang == 'fr') {
            tips = MICROPYTHON_TIPS_FR;
        }
        else { }
        //TODO: support other languages ...
    } else if (doc.getElementById('istm32')) {
        if (navLang == 'en' || navLang == 'en-US') {
            tips = STM_TIPS_EN;
        }
        else if (navLang == 'fr-FR' || navLang == 'fr') {
...
/**
 * Initialise Ace editor, set theme, handle features
 * @param  document 
 */
function editorInit(document) {
    var editor = ace.edit("editor");
    var langMode = ace.require("ace/mode/python").Mode;
    var langTools = ace.require("ace/ext/language_tools");
    editor.session.setMode(new langMode());
    editor.setOptions({
    ...
    var tipDict = generateDictionary(assignTips(document));
    var customCompleter = {
